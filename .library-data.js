...
/* ================= GIT RECOVERY ================= */
 {"id":"gitr-reflog","cat":"Git Recovery","title":"View the reflog to find lost commits","desc":"Show every position HEAD has held so you can locate commits lost to reset, rebase, or amend. Time refs like main@{yesterday} also work.","tags":["git","recovery","quick-win"],"code":{"ps":"git reflog\ngit reflog --date=iso -20","cmd":"git reflog\ngit reflog --date=iso -20","mac":"git reflog\ngit reflog --date=iso -20","linux":"git reflog\ngit reflog --date=iso -20"}},
 ...
 {"id":"gitr-fsck-lost-found","cat":"Git Recovery","title":"Find dangling objects with fsck","desc":"List dangling/unreachable commits, blobs, and trees — the pool git draws recoverable lost work from. '--lost-found' also writes them into .git/lost-found/.","tags":["git","recovery"],"code":{"ps":"git fsck --full --no-reflogs --unreachable\ngit fsck --lost-found","cmd":"git fsck --full --no-reflogs --unreachable\ngit fsck --lost-found","mac":"git fsck --full --no-reflogs --unreachable\ngit fsck --lost-found","linux":"git fsck --full --no-reflogs --unreachable\ngit fsck --lost-found"}},
...
/* ================= CONTAINERS ================= */
 {"id":"ctr-ps","cat":"Containers","title":"List containers","desc":"Show running and stopped containers. Requires docker or podman.","tags":["containers"],"code":{"ps":"docker ps -a   # podman ps -a","mac":"docker ps -a   # podman ps -a","linux":"docker ps -a   # podman ps -a"}},
 ...
 {"id":"ctr-run","cat":"Containers","title":"Run a container","desc":"Start an interactive throwaway container.","tags":["containers"],"code":{"ps":"docker run --rm -it {{IMAGE:alpine}} sh","mac":"docker run --rm -it {{IMAGE:alpine}} sh","linux":"docker run --rm -it {{IMAGE:alpine}} sh"}},
 ...
 {"id":"ctr-build","cat":"Containers","title":"Build an image","desc":"Build an image from a Dockerfile in the context dir.","tags":["containers"],"code":{"ps":"docker build -t {{TAG:myapp:latest}} {{CONTEXT:.}}","mac":"docker build -t {{TAG:myapp:latest}} {{CONTEXT:.}}","linux":"docker build -t {{TAG:myapp:latest}} {{CONTEXT:.}}"}},
 ...
 {"id":"ctr-k8s-get","cat":"Containers","title":"kubectl: list resources","desc":"List pods/nodes/services across namespaces. Requires kubectl.","tags":["containers"],"code":{"ps":"kubectl get pods -A   # also: nodes, svc, deploy","mac":"kubectl get pods -A   # also: nodes, svc, deploy","linux":"kubectl get pods -A   # also: nodes, svc, deploy"}},
